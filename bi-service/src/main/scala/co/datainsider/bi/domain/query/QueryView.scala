...
case class SqlView(aliasName: String, query: SqlQuery) extends QueryView

case class TableView(dbName: String, tblName: String) extends QueryView {
  val fullName: String = s"$dbName.$tblName"

  override val aliasName: String = "tbl_" + StringUtils.shortMd5(fullName)
}
