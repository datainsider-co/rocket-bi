...
import datainsider.client.service.{ProfileClientService, ScheduleClientService, SchemaClientService}
...
class AdminServiceImpl @Inject() (
  ...
  override def delete(request: DeleteUserDataRequest): Future[Boolean] = {
    logger.info(s"delete:: user org ${request.getOrganizationId()}, ${request.currentUsername} request delete user ${request.username}")
  ...
  private def handleDelete(orgId: OrganizationId, username: String): Future[Boolean] = {
    for {
      ...
      logger.info(
        s"Delete user $username in org $orgId result isDeletedUserData: ${isDeletedUserData}, isDeleteUser: ${isDeleteUser}"
      )
      true
    }
  }
  ...
  private def handleTransfer(orgId: OrganizationId, username: String, email: String): Future[Boolean] = {
    for {
      ...
      logger.info(
        s"Transfer user $username in org $orgId to $toUsername result isDeletedUserData: ${isDeletedUser}, isTransfer: ${isTransfer}"
      )
      true
    }
  }
  ...
  private def deleteUserData(orgId: OrganizationId, username: String): Future[Boolean] = {
    val result: Future[Boolean] = for {
      ...
      _ <- schemaClientService.deleteUserData(orgId, username)
      _ <- scheduleClientService.deleteUserData(orgId, username)
    } yield true
    result.rescue {
      case e: Exception =>
  ...
  private def transfer(orgId: OrganizationId, fromUsername: String, toUsername: String): Future[Boolean] = {
    val result: Future[Boolean] = for {
      ...
      _ <- schemaClientService.transfer(orgId, fromUsername, toUsername)
      _ <- scheduleClientService.transfer(orgId, fromUsername, toUsername)
    } yield true
    result.rescue {
      case ex: Exception =>
