...
case class ViewWidgetRightFilter @Inject() (
    ...
    Option(request.headerMap.getOrElse(tokenKey, null)) match {
      case Some(tokenId) =>
        ...
        widgetPermissionService
          ...
          .map {
            case true  => Permitted()
            case false => UnPermitted("Token no permission to view this widget.")
          }
      case None => Future.value(UnPermitted("Token is required to view this widget"))
    }
  }
  ...
  private def isUserPermitted(request: Request): Future[PermissionResult] = {
    if (request.isAuthenticated) {
      ...
      Future.value(UnPermitted("Login is required to view this widget."))
    }
  }
}
