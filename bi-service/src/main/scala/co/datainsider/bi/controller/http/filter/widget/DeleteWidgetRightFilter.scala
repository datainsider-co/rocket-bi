...
case class DeleteWidgetRightFilter @Inject() (
  ...
  def isTokenPermitted(request: Request): Future[PermissionResult] = {
    Option(request.headerMap.getOrElse(tokenKey, null)) match {
      case Some(tokenId) =>
        ...
        widgetPermissionService
          ...
          .map {
            case true  => Permitted()
            case false => UnPermitted("Token no permission to delete this widget.")
          }
      case None => Future.value(UnPermitted("Token is required to delete this widget"))
    }
  }
  ...
  private def isUserPermitted(request: Request): Future[PermissionResult] = {
    if (request.isAuthenticated) {
      ...
      widgetPermissionService
        ...
        .map {
          case true  => Permitted()
          case false => UnPermitted("No permission to delete this widget.")
        }
    } else {
      Future.value(UnPermitted("Login is required to delete this widget."))
    }
  }
}
