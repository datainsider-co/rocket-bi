  ...
  post(s"/share/:resource_type/:resource_id/action_permitted") { request: CheckActionPermittedRequest => {
      Profiler("[Controller::ShareController] DELETE /share/:resource_type/:resource_id/action_permitted") {
        ...
        DirectoryType.withName(request.resourceType) match {
          case DirectoryType.Directory => directoryPermissionService.isPermitted(orgId, username, request.resourceId.toLong, request.actions)
          case DirectoryType.Dashboard | DirectoryType.Queries => dashboardPermissionService.isPermitted(orgId, username, request.resourceId.toLong, request.actions)
