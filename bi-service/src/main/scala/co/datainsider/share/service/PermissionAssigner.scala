...
case class PermissionAssignerImpl @Inject() (
  ...
  override def assignRecurPermissions(
    ...
    for {
      childrenIds <- directoryService.listChildrenIds(dirId)
...
class MockPermissionAssigner extends PermissionAssigner {
  override def assignRecurPermissions(organizationId: DirectoryId, dirId: DirectoryId, userActionMap: Map[String, Seq[String]]): Future[Map[String, Boolean]] = Future.value(Map.empty)

  override def assignPermissions(organizationId: DirectoryId, dirIds: Seq[DirectoryId], username: String, actions: Set[String]): Future[Boolean] = Future.True

  override def assignRecurPermissions(organizationId: DirectoryId, dirId: DirectoryId, username: String, actions: Set[String]): Future[Boolean] = Future.True

  override def copySharedUserPermissions(organizationId: DirectoryId, childrenId: String, parentUserPermissions: Map[String, Seq[String]]): Future[Map[String, Boolean]] = Future.value(Map.empty)

  override def copyOwnerPermission(organizationId: DirectoryId, parentId: String, childrenId: String, ownerId: String): Future[Boolean] = Future.True
}
