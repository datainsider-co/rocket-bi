...
case class DirectoryPermissionServiceImpl @Inject() (
  ...
  private def buildPermissions(orgId: Long, id: DirectoryId, action: String): Future[Seq[String]] = {
    ...
    for {
      parentIds <- directoryService.listParentIds(id)
      parentPermissions =
        parentIds
          .map(parentId =>
  ...
  override def isPermitted(
    ...
    Profiler("[service::DirectoryPermissionService]::isPermittedActionByToken") {
      for {
        permissions <- buildPermissions(orgId, directoryId, action)
  ...
  override def isPermitted(
    ...
    Profiler("[service::DirectoryPermissionService]::isPermittedActions") {
      val fn = actions
        .map(action => {
  ...
  override def isPermitted(
    ...
    Profiler("[service::DirectoryPermissionService]::isPermittedActionsByToken") {
      val fn = actions
        .map(action => {
