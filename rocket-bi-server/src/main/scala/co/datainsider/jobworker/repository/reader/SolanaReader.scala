...
import co.datainsider.jobworker.repository.{SkippedBlockException, SolanaClient}
...
class SolanaReaderImpl(
  ...
  private var blockRecord: Record = Array.empty
    ...
    if (transactionNodes.isArray) {
      transactionNodes
        ...
        .forEach((node) => {
          ...
          val record: Array[Any] = this.transactionColumns.map(column => {
  ...
  private def toRewardRecords(rootData: JsonNode): Seq[Record] = {
    ...
    if (rewardNodes.isArray) {
      rewardNodes
        ...
        .forEach((node) => {
          val newNode = addExtraData(node, rootData)
          val record: Array[Any] = this.rewardColumns.map(column => {
