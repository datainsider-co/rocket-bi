...
import co.datainsider.jobworker.repository.reader.sqlserver.SqlServerReader
import co.datainsider.schema.domain.TableSchema
import co.datainsider.jobworker.domain.{DatabaseType, JdbcJob}
import co.datainsider.jobworker.repository.reader.{BigQueryReader, GenericJdbcReader, MySqlReader, OracleReader, PostgresReader, RedshiftReader}
...
object JdbcReader {
  def apply(source: JdbcSource, job: JdbcJob, batchSize: Int): JdbcReader = {
    source.databaseType match {
      ...
      case DatabaseType.Oracle    => reader.OracleReader(source, job, batchSize)
      case DatabaseType.SqlServer => SqlServerReader(source, job, batchSize)
      case DatabaseType.BigQuery  => reader.BigQueryReader(source, job, batchSize)
      case DatabaseType.Redshift  => reader.RedshiftReader(source, job, batchSize)
      case DatabaseType.Postgres  => reader.PostgresReader(source, job, batchSize)
