package co.datainsider.jobworker.service.hubspot.util
...
object ReflectionUtil {
  implicit class Reflector(ref: Any) {
    ...
    def setV(name: String, value: Any): Unit =
      ref.getClass.getMethods.find(_.getName == name).get.invoke(ref, value.asInstanceOf[AnyRef])
  }

  def getFieldsName[T: Manifest]: Iterable[String] =
    typeOf[T].members
      .filter(f => {
