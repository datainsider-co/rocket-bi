...
import co.datainsider.jobworker.service.hubspot.util.JsonUtil.ObjectLike
...
class APIKeyHubspotClient(
  ...
  def GET(url: String): Response[JsonNode] = http.GET(url)
}
...
abstract class HttpClient {
  ...
  def POST_MULTI[A: Manifest](
      path: String,
      data: Seq[MultiPartData],
      params: Seq[(String, String)] = Seq(),
      headers: Map[String, String] = Map()
  ): Response[A] = {
  ...
  protected def printRequest(req: HttpRequest, res: HttpResponse[String], body: String = ""): Unit = {
    if (isDebug) {
      ...
      println(s"-----------------[Request]------------------------")
      println(
        s"${req.method} ${req.url}${if (req.params.nonEmpty) s"?${req.params.map(f => s"${f._1}=${f._2}").mkString("&")}"
        else ""}"
      )
...
case class APIKeyHttpClient(hapiKey: String, connTimeout: Duration, readTimeout: Duration, debug: Boolean = false)
    extends HttpClient {
  ...
  override def makeHttpRequest(path: String, headers: Map[String, String] = Map()): HttpRequest = {
    ...
    Http(path)
      .timeout(connTimeoutMs = connTimeout.toMillis.toInt, readTimeoutMs = readTimeout.toMillis.toInt)
//      .param("hapikey", hapiKey)
      .header("authorization", "Bearer " + hapiKey)
      .headers(_headers)
  }
}

case class OAuthHttpClient(
  ...
  private[this] def verifyToken(): Unit = {
    ...
    (if (accessToken.get() == null || accessToken.get().isEmpty)
       refreshAccessToken(tokenUri, clientId, clientSecret, refreshToken)
     else {
       val accessTokenInfo = getAccessTokenInfo(accessToken.get())
       if (accessTokenInfo.isEmpty) refreshAccessToken(tokenUri, clientId, clientSecret, refreshToken)
       else accessTokenInfo
     }) match {
      case Some(x) =>
  ...
  private def getRefreshTokenInfo(refreshToken: String): Option[RefreshToken] = {
    val res = Http(s"$refreshTokenInfoUri/$refreshToken")
      .timeout(connTimeout.toMillis.toInt, readTimeout.toMillis.toInt)
      .asString
  ...
  private def getAccessTokenInfo(accessToken: String): Option[AccessToken] = {
    val res =
      Http(s"$accessTokenInfoUri/$accessToken").timeout(connTimeout.toMillis.toInt, readTimeout.toMillis.toInt).asString
...
case class BasicHttpClient(debug: Boolean = false, connTimeoutMs: Int = 5000, readTimeoutMs: Int = 10000)
    extends HttpClient {
  override def makeHttpRequest(path: String, headers: Map[String, String] = Map()): HttpRequest = {
    val _headers = Map(
      "Content-Type" -> "application/json;charset=utf-8"
