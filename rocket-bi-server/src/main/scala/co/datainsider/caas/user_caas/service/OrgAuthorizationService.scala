...
import datainsider.client.exception.{BadRequestError, InsufficientPermissionError}
...
case class OrgAuthorizationServiceImpl @Inject() (
  ...
  private def verifyNumEditors(orgId: Long): Future[Boolean] = {
    for {
      ...
      licenseKey <- organizationService.getOrganization(orgId).map(_.get.licenceKey.get)
