...
case class SendGridEmailService(sendGrid: SendGrid, sender: String) extends ChannelService with Logging {
  ...
  override def sendHtmlMessage(email: String, subject: String, html: String): Future[Unit] = {
    Future {
      ...
      try {
        ...
        val mail = new Mail(new Email(sender), subject, new Email(email), content)
