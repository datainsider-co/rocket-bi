...
class DeleteAccessFilterImpl @Inject() (
    caasService: CaasService
) extends UserAccessFilters.DeleteAccessFilter
    with BaseUserFilter {

  override protected def getValidatorChain(): Seq[AccessValidator] = {
    Seq(isNotMe(), isPermissionPermitted)
  }
  ...
  private def isPermissionPermitted(request: Request) = {
    ...
    caasService
      ...
      .isPermitted(
        ...
        PermissionProviders.user.withOrganizationId(organizationId).delete()
      )
      .map(PermissionResult(_, "No permission to delete users."))
  }
}
