...
case class KeyStoreConfiguration(
  ...
  def credentialPath: String = {
    val baseSSLDir: String = ZConfig.getString("data_cook.ssl_dir", "./tmp/ssl")
    val hashName: String = MurmurHash3.stringHash(data).toString
    val finalFileName: String = s"${hashName}_${fileName}"
    val path = Paths.get(baseSSLDir, organizationId.toString, `type`, finalFileName).toAbsolutePath.toString
