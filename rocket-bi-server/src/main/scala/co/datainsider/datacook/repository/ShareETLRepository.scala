...
import co.datainsider.schema.repository.ObjectShareRepository.EnhanceImplicitResultSet
...
class ShareETLRepositoryImpl(client: JdbcClient, dbName: String, tblName: String, generator: IDGenerator[Int])
    extends ShareETLRepository {
  ...
  override def softDelete(
    ...
    val data: Array[Array[Any]] = usernames
  ...
  override def updateUpdatedTimeShareInfo(shareIds: Seq[String]): Future[Boolean] = {
    val shareData: Array[Array[Any]] = shareIds.map(shareId => Array(System.currentTimeMillis(), shareId)).toArray
    val r: Boolean = client.executeBatchUpdate(
      s"""
         |UPDATE $getTable
         |SET updated_at = ?
