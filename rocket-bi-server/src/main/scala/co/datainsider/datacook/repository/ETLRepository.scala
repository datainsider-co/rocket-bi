...
import co.datainsider.datacook.domain.request.etl.{CreateEtlJobRequest, ListEtlJobsRequest}
import co.datainsider.datacook.domain.{EtlJob, ETLStatus}
...
class ETLRepositoryImpl(client: JdbcClient, dbName: String, tblName: String, shareTblName: String)
    extends ETLRepository {
  ...
  override def getNextJob: Future[Option[EtlJob]] =
    Future {
      ...
      client.executeQuery(query, ETLStatus.Queued.toString, ETLStatus.Running.toString)(toEtlJobs).headOption
    }
  ...
  override def migrateData(jobs: Seq[EtlJob]): Future[Unit] =
    Future {
      ...
      val records: Array[Array[Any]] = jobs
        .map(job =>
          Array(
            JsonParser.toJson(job.operatorInfo, false),
            JsonParser.toJson(job.config, false),
  ...
  override def listSharedJobs(
      organizationId: OrganizationId,
      request: ListEtlJobsRequest
  ): Future[(EtlJobId, Seq[EtlJob])] =
    Future {
