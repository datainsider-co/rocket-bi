...
import co.datainsider.jobscheduler.domain.{JobProgress, RangeValue, ShopeeJobProgress}
...
case class ShopeeJob(
  ...
  override def copyJobStatus(progress: JobProgress): Job = {
    val lastSyncedValue = progress match {
      case jobProgress: ShopeeJobProgress => jobProgress.lastSyncedValue
      case _                                 => None
    }
...
object ShopeeJob {
  def fromResultSet(rs: ResultSet): ShopeeJob = {
    ...
    ShopeeJob(
      ...
      shopId = jobData.getString("shop_id"),
