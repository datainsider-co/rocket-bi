...
object MySqlParser extends SqlParser {
  ...
  def toQueryString(scalarFn: ScalarFunction, field: String): String = {
    scalarFn match {
      ...
      case ToDateTime(_) => s"CONVERT($field, DATETIME)"

      case SecondsToDateTime(_) => s"FROM_UNIXTIME($field)"
      case MillisToDateTime(_)  => s"FROM_UNIXTIME($field / 1000)"
      case NanosToDateTime(_)   => s"FROM_UNIXTIME($field / 1000000)"
      case DatetimeToSeconds(_) => s"UNIX_TIMESTAMP($field)"
      case DatetimeToMillis(_)  => s"UNIX_TIMESTAMP($field) * 1000"
      case DatetimeToNanos(_)   => s"UNIX_TIMESTAMP($field) * 1000000"
      ...
      case ToSecondNum(_)  => s"UNIX_TIMESTAMP($field)"

      case DateDiff(unit, date, _)   => s"TIMESTAMPDIFF($unit, $date, $field)"
      case GetArrayElement(_, index) => ???
      case Decrypt(_)                => ???

      case PastNYear(unit, _)    => s"DATE_SUB($field, INTERVAL $unit YEAR)"
      case PastNQuarter(unit, _) => s"DATE_SUB($field, INTERVAL $unit QUARTER)"
      case PastNMonth(unit, _)   => s"DATE_SUB($field, INTERVAL $unit MONTH)"
      case PastNWeek(unit, _)    => s"DATE_SUB($field, INTERVAL $unit WEEK)"
      case PastNDay(unit, _)     => s"DATE_SUB($field, INTERVAL $unit DAY)"
