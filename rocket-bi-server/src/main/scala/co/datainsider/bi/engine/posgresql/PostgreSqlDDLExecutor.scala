...
/**
  ...
  */
class PostgreSqlDDLExecutor(val client: JdbcClient) extends DDLExecutor with Logging {
  ...
  override def dropDatabase(dbName: String): Future[Boolean] =
    Future {
      val query = s"DROP SCHEMA IF EXISTS ${dbName} CASCADE"
  ...
  private def getColumns(dbName: String, tblName: String): Seq[Column] = {
    ...
    client.executeQuery(listColumnsQuery, dbName, tblName)(rs => {
      ...
      while (rs.next()) {
        ...
        val column = PostgreSqlUtils.toColumn(columnName, columnType, isNullable, defaultValue)
        columns += column
      }
      columns
    })
  ...
  override def dropTable(dbName: String, tblName: String): Future[Boolean] =
    Future {
      val query = s"DROP TABLE IF EXISTS ${dbName}.${tblName} CASCADE"
      client.executeUpdate(query) >= 0
    }
  ...
  override def detectColumns(query: String): Future[Seq[Column]] =
    Future {
      try {
        ...
        client.executeQuery(finalQuery)((rs: ResultSet) => PostgreSqlUtils.parseColumns(rs.getMetaData))
      } catch {
