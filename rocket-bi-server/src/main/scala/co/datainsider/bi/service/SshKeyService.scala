...
class SshServiceImpl (sshRepository: SshKeyRepository) extends SshKeyService with Logging {
  private val PUBLIC_KEY_COMMENT = "from rocket.bi"
  private val clazz: String = getClass.getSimpleName

  def createKey(orgId: Long): Future[Boolean] = Profiler(s"[Service] $clazz::createKey") {
  ...
  /**
    ...
    * @return private key, public key, passphrase
    */
  override def generateKeyPair(orgId: Long, keyLength: Int = 2048): Future[SshKeyPair] = Profiler(s"[Service] $clazz::generateKeyPair") {
    Future {
      var keyPair: KeyPair = null
      try {
        val passphrase = UUID.randomUUID().toString
        val jsch = new JSch()
        val keyPair = KeyPair.genKeyPair(jsch, KeyPair.RSA, keyLength)
  ...
  override def getKeyPair(orgId: Long): Future[SshKeyPair] = Profiler(s"[Service] $clazz::getKeyPair") {
...
case class CacheSshService(service: SshKeyService) extends SshKeyService {
  ...
  override def generateKeyPair(orgId: Long, keyLength: Int): Future[SshKeyPair] = service.generateKeyPair(orgId, keyLength)

  override def getKeyPair(orgId: Long): Future[SshKeyPair] = Implicits.async {
    keyPairMap.computeIfAbsent(orgId, (_) => {
