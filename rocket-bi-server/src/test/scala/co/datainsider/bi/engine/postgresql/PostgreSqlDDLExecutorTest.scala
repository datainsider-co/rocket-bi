...
/**
  ...
  */
class PostgreSqlDDLExecutorTest extends Test {
  val injector = TestInjector(TestContainerModule).create
  val source: PostgreSqlConnection = injector.instance[PostgreSqlConnection]
  val engine = new PostgreSqlEngine(new ClientManager())
  val dbName = "test"
