...
class BasePostgreSqlTest extends IntegrationTest {
  ...
  val engine: PostgreSqlEngine = new PostgreSqlEngine(new ClientManager())

  val client: JdbcClient = engine.createClient(postgresSource)
  ...
  test("test query postgres") {
    ...
    val dataTable = engine.execute(postgresSource, sql).syncGet()
