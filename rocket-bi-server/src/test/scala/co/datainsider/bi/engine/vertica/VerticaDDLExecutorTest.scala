...
/**
  ...
  */
class VerticaDDLExecutorTest extends IntegrationTest {
  val injector = TestInjector(TestContainerModule).create
  val source = injector.instance[VerticaConnection]
  val engine = new VerticaEngine(new ClientManager())
  val dbName = "testdb"
  var tableName = "default_table"
  var viewTblName = "view_table"
  var materializedViewTblName = "materialized_view_table"
  val client = engine.createClient(source)
  val ddlExecutor = engine.getDDLExecutor(source)
  private val orgId = 1
  ...
  private def cleanup(): Unit = {
    try {
      client.executeUpdate(s"""DROP SCHEMA IF EXISTS "${dbName}" CASCADE """)
    } catch {
  ...
  test("[View Table] test rename table") {
    assertFailedFuture[InternalError](ddlExecutor.renameTable(dbName, viewTblName, "new_view_table"))
  }
  ...
  test("[Default Table] test update column int to string") {
    ...
    assertFailedFuture[InternalError](ddlExecutor.updateColumn(dbName, tableName, newColumn))
  }
  ...
  test("[Detect columns] text, number and date time") {
    val query =
      "select 'text' as text, 1 as number, CAST('99999999999999999999999999' as NUMBER) as long_number, CAST('2020-01-01 00:00:00' as DATETIME) as datetime"
    ...
    assert(longNumberColumn.getClass == classOf[DoubleColumn])
  ...
  private def assertColumn(tableType: TableType, actualColumn: Column, expectedColumn: Column): Unit = {
    expectedColumn match {
      ...
      case _: Int8Column   => assert(actualColumn.isInstanceOf[Int64Column])
      case _: Int16Column  => assert(actualColumn.isInstanceOf[Int64Column])
      case _: Int32Column  => assert(actualColumn.isInstanceOf[Int64Column])
      case _: UInt8Column  => assert(actualColumn.isInstanceOf[Int64Column])
      case _: UInt16Column => assert(actualColumn.isInstanceOf[Int64Column])
      case _: UInt32Column => assert(actualColumn.isInstanceOf[Int64Column])
      case _: UInt64Column => assert(actualColumn.isInstanceOf[Int64Column])
