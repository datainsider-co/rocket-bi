...
class BigQueryEngineTest extends Test {
  ...
  val ddlExecutor = engine.getDDLExecutor(bqSource)
  ...
  override def beforeAll(): Unit = {
    ...
    engine.createClient(bqSource).loadJsonFile(path, tableSchema.dbName, tableSchema.name)
  }
  ...
  test("test connection success") {
    val isSuccess: Boolean = await(engine.testConnection(bqSource))
    assert(isSuccess)
  }

  test("test connection failed") {
    val source = bqSource.copy(projectId = "wrong_project_id")
    assertFailedFuture[Throwable](engine.testConnection(source))
  }
  ...
  test("test export to csv") {
    ...
    val destPath = engine.exportToFile(bqSource, sql, "./bq_sales.csv", FileType.Csv).syncGet()
  ...
  test("test export to excel") {
    ...
    val destPath = engine.exportToFile(bqSource, sql, "./bq_sales.xlsx", FileType.Excel).syncGet()
    val file = new File(destPath)
    assert(file.exists())
    assert(file.length() > 0)
    file.delete()
  ...
  test("test write data to big query") {
    ...
    val insertedRows = engine.write(bqSource, tableSchema, records).syncGet()
