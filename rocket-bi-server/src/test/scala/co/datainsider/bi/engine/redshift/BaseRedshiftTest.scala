...
class BaseRedshiftTest extends IntegrationTest {
  ...
  val engine: RedshiftEngine = new RedshiftEngine(new ClientManager())

  val client: JdbcClient = engine.createClient(redshiftSource)
  ...
  test("test query redshift") {
    ...
    val dataTable = engine.execute(redshiftSource, sql).syncGet()
