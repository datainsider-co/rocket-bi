...
class MySqlEngineTest extends BaseMySqlTest {
  ...
  test("test export csv") {
    ...
    val destPath = engine.exportToFile(source, sql, csvPath, FileType.Csv).syncGet()
  ...
  test("test detect aggregate expression column success") {
    ...
    val column = await(engine.detectAggregateExpressionColumn(source, dbName, tblName, expression, Map.empty))
    assert(column != null)
    assert(column.getClass == classOf[DoubleColumn])
  }
  ...
  test("test detect aggregate expression column with invalid expression") {
    ...
    assertFailedFuture[DbExecuteError](
      engine.detectAggregateExpressionColumn(source, dbName, tblName, expression, Map.empty)
    )
  }
  ...
  test("test query histogram") {
    ...
    val dataTable: DataTable = engine.execute(source, histogramSql).syncGet()
