...
import co.datainsider.bi.engine.bigquery.BigQueryParser
...
class MySqlQueryParserTest extends BaseMySqlTest {
  val parser = new QueryParserImpl(MySqlParser)
  ...
  test("test parse select query") {
    ...
    val dataTable: DataTable = engine.execute(source, sql).syncGet()
