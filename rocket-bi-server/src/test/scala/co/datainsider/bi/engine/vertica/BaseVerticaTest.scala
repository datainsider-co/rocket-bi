...
/**
 * created 2023-07-18 5:19 PM
 *
 * @author tvc12 - Thien Vi
 */
abstract class BaseVerticaTest extends IntegrationTest {
  override protected val injector: Injector = TestInjector(TestContainerModule).create

  val source: VerticaConnection = injector.instance[VerticaConnection]

  val engine: VerticaEngine = new VerticaEngine(new ClientManager())
  ...
  def setupSampleData(): Unit = {
    client.executeUpdate(s"""CREATE SCHEMA IF NOT EXISTS "$dbName" """)
