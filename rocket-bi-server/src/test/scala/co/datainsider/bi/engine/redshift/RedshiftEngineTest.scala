...
class RedshiftEngineTest extends BaseRedshiftTest {
  ...
  override def beforeAll(): Unit = {
    ...
    await(engine.getDDLExecutor(redshiftSource).createTable(studentsSchema))
  }
  ...
  test("test export csv") {
    ...
    val destPath = engine.exportToFile(redshiftSource, sql, csvPath, FileType.Csv).syncGet()
  ...
  test("test export excel") {
    ...
    val destPath = engine.exportToFile(redshiftSource, sql, excelPath, FileType.Excel).syncGet()
    val file = new File(destPath)
    assert(file.exists())
    assert(file.length() > 0)
    file.delete()
  ...
  test("test query histogram") {
    ...
    val dataTable: DataTable = engine.execute(redshiftSource, histogramSql).syncGet()
  ...
  test("test detect expression column success") {
    ...
    val column = await(engine.detectExpressionColumn(redshiftSource, dbName, tblName, expression, Map.empty))
    assert(column != null)
  ...
  test("test detect expression column with invalid expression") {
    ...
    assertFailedFuture[DbExecuteError](
      engine.detectExpressionColumn(redshiftSource, dbName, tblName, expression, Map.empty)
    )
  }

  test("test detect aggregate expression column success") {
    val expression = "sum(Total_Profit)"
    val column = await(engine.detectAggregateExpressionColumn(redshiftSource, dbName, tblName, expression, Map.empty))
    assert(column != null)
    assert(column.getClass == classOf[DoubleColumn])
  }
  ...
  test("test detect aggregate expression column with invalid expression") {
    val expression = "sum(Total_Profit) +"
    assertFailedFuture[DbExecuteError](
      engine.detectAggregateExpressionColumn(redshiftSource, dbName, tblName, expression, Map.empty)
    )
  }
  ...
  test("test write data") {
    ...
    await(engine.write(redshiftSource, studentsSchema, records))
    val query =
      s"""SELECT *
