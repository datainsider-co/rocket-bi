...
class DrillThroughRepositoryTest extends IntegrationTest {
  ...
  private lazy val dashboardFieldRepository: DrillThroughFieldRepository =
    injector.instance[DrillThroughFieldRepository]
  ...
  test("list dashboard drill through") {
    val dashboards: PageResult[Dashboard] = await(
      dashboardRepository.listDashboards(
        orgId,
        new ListDrillThroughDashboardRequest(
          Array(field, field2),
          excludeIds = Array(1),
          isRemoved = Some(false),
          from = 0,
          size = 20,
          request = MockUserContext.getLoggedInRequest(1, "tvc12")
        )
      )
    )
