...
import co.datainsider.jobworker.repository.reader.lazada.OrderLazadaReader
...
/**
  ...
  */
class OrderLazadaReaderTest extends AbstractShopeeReaderTest {
  ...
  val baseJob = new LazadaJob(
    ...
    tableName = LazadaSupportedTable.Order,
  ...
  test("init reader must success") {
    val reader = new OrderLazadaReader(client, timeRange, job = baseJob)
    assert(reader != null)
  ...
  test("read data must success") {
    Using(new OrderLazadaReader(client, timeRange, baseJob)) { reader =>
      {
