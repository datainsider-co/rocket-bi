...
import co.datainsider.jobworker.repository.reader.lazada.OrderItemLazadaReader
...
/**
  ...
  */
class OrderItemLazadaReaderTest extends AbstractShopeeReaderTest {
  ...
  val baseJob = new LazadaJob(
    ...
    tableName = LazadaSupportedTable.OrderItem,
  ...
  test("init reader must success") {
    val reader = new OrderItemLazadaReader(client, timeRange, job = baseJob)
    assert(reader != null)
  ...
  test("read data must success") {
    Using(new OrderItemLazadaReader(client, timeRange, baseJob)) { reader =>
      {
