...
import co.datainsider.jobworker.repository.reader.lazada.PartnerTransactionLazadaReader
...
/**
  ...
  */
class PartnerTransactionLazadaReaderTest extends AbstractShopeeReaderTest {
  ...
  val baseJob = new LazadaJob(
    ...
    tableName = LazadaSupportedTable.PartnerTransaction,
  ...
  test("init reader must success") {
    val reader = new PartnerTransactionLazadaReader(client, timeRange, job = baseJob)
    assert(reader != null)
  ...
  test("read data must success") {
    Using(new PartnerTransactionLazadaReader(client, timeRange, baseJob)) { reader =>
      {
        ...
        reader.columns.zipWithIndex.foreach {
          case (column, index) =>
            ...
            ensureExistsValue(value, column, Set("updated_at", "created_at", "voucher_platform"))
        }
