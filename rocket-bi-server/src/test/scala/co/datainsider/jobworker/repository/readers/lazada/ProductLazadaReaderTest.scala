...
import co.datainsider.jobworker.repository.reader.lazada.ProductLazadaReader
...
/**
  ...
  */
class ProductLazadaReaderTest extends AbstractShopeeReaderTest {
  ...
  val baseJob = new LazadaJob(
    ...
    tableName = LazadaSupportedTable.Product,
  ...
  test("init reader must success") {
    val reader = new ProductLazadaReader(client, timeRange, job = baseJob)
    assert(reader != null)
  ...
  test("read data must success") {
    Using(new ProductLazadaReader(client, timeRange, baseJob)) { reader =>
      {
