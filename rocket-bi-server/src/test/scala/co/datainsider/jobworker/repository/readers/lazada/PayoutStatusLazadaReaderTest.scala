...
import co.datainsider.jobworker.repository.reader.lazada.PayoutStatusLazadaReader
...
/**
  ...
  */
class PayoutStatusLazadaReaderTest extends AbstractShopeeReaderTest {
  ...
  val baseJob = new LazadaJob(
    ...
    tableName = LazadaSupportedTable.PayoutStatus,
  ...
  test("init reader must success") {
    val reader = new PayoutStatusLazadaReader(client, timeRange, job = baseJob)
    assert(reader != null)
  ...
  test("read data must success") {
    Using(new PayoutStatusLazadaReader(client, timeRange, baseJob)) { reader =>
      {
        ...
        reader.columns.zipWithIndex.foreach {
          case (column, index) =>
            ...
            ensureExistsValue(value, column, Set("subtotal2", "subtotal1", "created_at"))
        }
