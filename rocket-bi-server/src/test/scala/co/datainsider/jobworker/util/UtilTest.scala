...
import co.datainsider.jobworker.domain.{DataSource, JdbcProgress, JobStatus}
...
class UtilTest extends Test {
  ...
  test("json serializer") {
    ...
    val jobResp: NextJobResponse = util.JsonUtils.fromJson[NextJobResponse](json)
    assert(jobResp != null)
    println(jobResp)
  }
  ...
  test("serialize progress") {
    ...
    val json = util.JsonUtils.toJson(progress)
    println(json)
  }
  ...
  test("deserialize data source") {
    ...
    val oracle = util.JsonUtils.fromJson[DataSource](json)
    println(oracle)
  }
  ...
  test("test get client id from service account") {
    ...
    val serviceAccountJson = util.JsonUtils.fromJson[JsonNode](serviceAccount)
