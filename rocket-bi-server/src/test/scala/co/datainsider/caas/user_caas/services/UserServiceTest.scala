...
class UserServiceTest extends DataInsiderIntegrationTest {
  ...
  test("update user properties, drop properties") {
    val newProfile = await(
      adminService.updateUserProperties(orgId, username, Map("firstName" -> "Thien", "class" -> "Vi"), Set("firstName"))
    )
  ...
  test("update user properties, drop second properties") {
    ...
    val newProfile =
      await(adminService.updateUserProperties(orgId, username, Map("drop" -> "32", "drop_1" -> "1"), Set("drop")))
