...
import co.datainsider.jobscheduler.domain.job.{JobStatus, LazadaJob}
...
class LazadaJobTest extends IntegrationTest with BeforeAndAfterAll {
  ...
  private def assertJob(baseJob: LazadaJob, expected: LazadaJob): Unit = {
  ...
  test("create job success") {
    ...
    val resultJob = newJob.job.asInstanceOf[LazadaJob]
  ...
  test("test get job") {
    ...
    val resultJob = jobInfo.job.asInstanceOf[LazadaJob]
    assertJob(resultJob, expectedJob)
  }
  ...
  test("test update job") {
    ...
    val resultJob = updatedJob.job.asInstanceOf[LazadaJob]
