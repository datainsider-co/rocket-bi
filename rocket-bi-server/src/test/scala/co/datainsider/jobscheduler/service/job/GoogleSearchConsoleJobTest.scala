...
import co.datainsider.jobscheduler.domain.source.GoogleSearchConsoleSource
...
class GoogleSearchConsoleJobTest extends IntegrationTest with BeforeAndAfterAll {
  ...
  val dataSource = GoogleSearchConsoleSource(
    orgId = 1L,
    id = 1L,
    displayName = "source",
    creatorId = "creator-id",
    refreshToken = "refresh-token",
    accessToken = "access-toen"
  )

  var jobId: JobId = 0L
  var expectedJob = GoogleSearchConsoleJob(
    orgId = 1,
    jobId = 1L,
    displayName = "sad",
    sourceId = 0L,
  ...
  private def assertJob(baseJob: GoogleSearchConsoleJob, expected: GoogleSearchConsoleJob): Unit = {
    ...
    assert(baseJob.siteUrl == expected.siteUrl)
    assert(baseJob.tableType == expected.tableType)
  }
  ...
  test("create job success") {
    ...
    val resultJob = newJob.job.asInstanceOf[GoogleSearchConsoleJob]
  ...
  test("test get job") {
    ...
    val resultJob = jobInfo.job.asInstanceOf[GoogleSearchConsoleJob]
    assertJob(resultJob, expectedJob)
  }
  ...
  test("test update job") {
    val newJob = expectedJob.copy(
      ...
      siteUrl = "https://www.google.com/new",
      tableType = GoogleSearchConsoleType.SearchAppearance,
      searchAnalyticsConfig = SearchAnalyticsConfig(SearchAnalyticsType.News)
    )
    ...
    val resultJob = updatedJob.job.asInstanceOf[GoogleSearchConsoleJob]
