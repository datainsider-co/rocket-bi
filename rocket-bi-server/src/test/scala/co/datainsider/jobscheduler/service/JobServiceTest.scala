...
import co.datainsider.jobscheduler.domain.source.{AmazonS3Source, DataSource, JdbcSource, MongoSource}
import co.datainsider.jobscheduler.domain.{CsvConfig, DatabaseType}
...
import co.datainsider.schema.domain.column.{Int32Column, StringColumn}
import com.google.inject.name.Names
import com.twitter.inject.app.TestInjector
import com.twitter.inject.{Injector, IntegrationTest}
import com.twitter.util.Await
import datainsider.client.domain.scheduler.{ScheduleHourly, ScheduleMinutely, ScheduleOnce}
import datainsider.client.util.{JsonParser, TimeUtils}
...
class JobServiceTest extends IntegrationTest with BeforeAndAfterAll {
  ...
  test("test create google ads job") {
    ...
    assert(result.job.isInstanceOf[GoogleAdsJob])
    val jobAsJson = JsonParser.toJson(result.job.asInstanceOf[GoogleAdsJob].copy(lastModified = 0), false)
    assert(jobAsJson == JsonParser.toJson(job.copy(jobId = googleAdsJobId), false))
  }
  ...
  test("test update google ads job") {
    ...
    assert(updatedJob.isInstanceOf[GoogleAdsJob])
  }
