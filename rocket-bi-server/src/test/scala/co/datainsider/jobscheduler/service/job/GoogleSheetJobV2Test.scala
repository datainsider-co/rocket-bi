...
import co.datainsider.jobscheduler.domain.job.{GoogleSheetJobV2, JobStatus}
...
import co.datainsider.jobscheduler.domain.source.GoogleSheetSource
...
class GoogleSheetJobV2Test extends IntegrationTest with BeforeAndAfterAll {
  ...
  test("test create GoogleSheetJobV2") {
    ...
    dataSource =
      await(sourceService.get(googleSheetJobV2.orgId, googleSheetJobV2.sourceId)).get.asInstanceOf[GoogleSheetSource]
  ...
  test("test update google sheet job v2") {
    val isSuccess = await(
      jobService.update(
        googleSheetJobV2.orgId,
        UpdateJobRequest(
          id = googleSheetJobV2.jobId,
          job = googleSheetJobV2.copy(sheetId = 10, spreadSheetId = "wqewqcq123"),
          request = null
        )
      )
    )
