...
import co.datainsider.jobscheduler.domain.source.{DataSource, GoogleSearchConsoleSource, PalexySource}
...
class GoogleSearchConsoleSourceTest extends IntegrationTest with BeforeAndAfterAll {
  ...
  val expectedSource = GoogleSearchConsoleSource(
    orgId = 11,
    id = 1,
    displayName = "name",
    creatorId = "tvc12",
  ...
  test("test create source success") {
    ...
    val resultSource = result.get.asInstanceOf[GoogleSearchConsoleSource]
    newSourceId = result.get.getId
    assertSource(resultSource, expectedSource)
  }

  private def assertSource(resultSource: GoogleSearchConsoleSource, expectedSource: GoogleSearchConsoleSource): Unit = {
  ...
  test("test get source success") {
    ...
    val resultSource = dataSource.get.asInstanceOf[GoogleSearchConsoleSource]
    assertSource(resultSource, expectedSource)
  }
