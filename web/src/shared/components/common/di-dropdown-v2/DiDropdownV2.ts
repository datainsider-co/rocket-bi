...
export default class DiDropdownV2 extends Vue {
  private keyword = '';
  ...
  private get popoverClass(): string {
    if (this.appendAtRoot) {
      return 'select-area-popover-v2';
    } else {
      return 'select-area-popover-v2 w-100';
    }
  }
  ...
  private showDropdown(): void {
    ...
    this.$root.$emit(DiDropdownV2.SHOWN_KEY, this.id);
  }
  ...
  mounted() {
    this.$root.$on(DiDropdownV2.SHOWN_KEY, this.handleDropdownShown);
  }

  beforeDestroy() {
    this.$root.$off(DiDropdownV2.SHOWN_KEY, this.handleDropdownShown);
  }
