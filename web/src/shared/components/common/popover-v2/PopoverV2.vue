...
export default class PopoverV2 extends Vue {
  ...
  private handleClickOutside(e: MouseEvent) {
    try {
      // Log.debug('PopoverV2::handleClickOutside::', e);
      if (this.hideOnBackdrop && this.showing && e.target && !window.$.contains(this.$el, e.target) && !this.isIgnoredTarget(e.target as Element)) {
        this.hidePopover();
      }
    } catch (ex) {
      Log.error('PopoverV2::handleClickOutside::', ex);
    }
  }

  private isIgnoredTarget(element: Element) {
    const intersectedClasses = intersection(this.ignoredClasses, element.classList);
    return intersectedClasses.length > 0 ? true : false;
  }
