...
import { MySqlConnector, SSHConfig } from '@core/connector-config';
...
@Component({
  components: { DiInputComponent2, DiIconTextButton, DiToggle, SSHTunnel, AddPropertiesComponent },
  validations: {
    source: {
      host: { required },
      port: { required },
...
export default class MySQLConnectorForm extends AbstractConnectorForm<MySqlConnector> {
  @PropSync('model')
  ...
  mounted() {
    Log.debug('MySQLSourceForm::mounted');
  ...
  setSSHConfig() {
    Log.debug('MySQLSourceConfig::setSSHConfig::', this.sshConfigComponent, this.source.tunnelConfig);
