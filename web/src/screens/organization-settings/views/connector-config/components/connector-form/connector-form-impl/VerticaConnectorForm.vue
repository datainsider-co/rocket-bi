...
import { SSHConfig, VerticaConnector } from '@core/connector-config';
...
@Component({
  components: { AddPropertiesComponent, DiInputComponent2, DiIconTextButton, DiToggle, SSHTunnel, ManagePropertyModal },
  validations: {
    source: {
      host: { required },
      port: { required },
...
export default class VerticaConnectorForm extends AbstractConnectorForm<VerticaConnector> {
  @PropSync('model')
  protected source!: VerticaConnector;

  protected propertyDataList: AddPropertyData[] = [];

  @Ref()
  protected readonly sshConfigComponent?: SSHTunnel;
  ...
  setSSHConfig() {
    Log.debug('VerticaSourceConfig::setSSHConfig::', this.sshConfigComponent, this.source.tunnelConfig);
