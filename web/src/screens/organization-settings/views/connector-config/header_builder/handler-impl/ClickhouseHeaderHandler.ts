...
import { ClickhouseConnector } from '@core/connector-config';
import { CustomCell, HeaderData } from '@/shared/models';
import { HtmlElementRenderUtils } from '@/utils';

export class ClickhouseHeaderHandler extends DataSourceHeaderHandler<ClickhouseConnector> {
  buildHeader(source: ClickhouseConnector): HeaderData[] {
    return [
      {
        key: 'host',
        label: 'Host',
        customRenderBodyCell: new CustomCell(rowData => {
          const source = rowData?.source ? ClickhouseConnector.fromObject(rowData.source) : null;
