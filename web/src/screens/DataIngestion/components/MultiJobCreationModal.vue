          ...
          <div class="job-section">
            <div class="input">
              ...
              <template v-if="isJdbcJob">
                <MultiJdbcFromDatabaseSuggestion
              ...
              <template v-if="isGenericJdbcJob">
                <MultiGenericJdbcFromDatabaseSuggestion
              ...
              <template v-if="isMongoJob">
                <MultiMongoFromDatabaseSuggestion
              ...
              <template v-if="isBigQueryJob">
                <MultiBigQueryFromDatabaseSuggestion
              ...
              <template v-if="isShopifyJob">
                <MultiShopifyFromDatabaseSuggestion
...
import { DataDestination, DataSourceInfo, DataSources, DataSourceType, FormMode, JdbcJob, Job, JobType, S3Job, SyncMode } from '@core/DataIngestion';
import { DataSourceModule } from '@/screens/DataIngestion/store/DataSourceStore';
import { DatabaseCreateRequest, DIException } from '@core/domain';
...
import JdbcDatabaseSuggestion from '@/screens/DataIngestion/components/JdbcFromDatabaseSuggestion.vue';
import MultiBigQueryFromDatabaseSuggestion from '@/screens/DataIngestion/components/MultiBigQueryFromDatabaseSuggestion.vue';
import { BigQueryJob } from '@core/DataIngestion/Domain/Job/BigQueryJob';
import BigQueryExtraForm from '@/screens/DataIngestion/BigQueryExtraForm.vue';
import { BigQuerySourceInfoV2 } from '@core/DataIngestion/Domain/DataSource/BigQuerySourceInfoV2';
import MultiMongoFromDatabaseSuggestion from '@/screens/DataIngestion/MongoJobForm/MultiMongoFromDatabaseSuggestion.vue';
import { MongoJob } from '@core/DataIngestion/Domain/Job/MongoJob';
import MongoDepthConfig from '@/screens/DataIngestion/MongoJobForm/MongoDepthConfig.vue';
import MultiGenericJdbcFromDatabaseSuggestion from '@/screens/DataIngestion/components/GenericJdbcJobForm/MultiGenericJdbcFromDatabaseSuggestion.vue';
import { GenericJdbcJob } from '@core/DataIngestion/Domain/Job/GenericJdbcJob';
import JobLakeConfig from '@/screens/DataIngestion/components/LakeConfig/JobLakeConfig.vue';
import JobSyncConfig from '@/screens/DataIngestion/components/SyncConfg/JobSyncConfig.vue';
import DataSourceConfig from '@/screens/DataIngestion/components/DataSourceConfig.vue';
import JobWareHouseConfig from '@/screens/DataIngestion/components/WareHouseConfig/JobWareHouseConfig.vue';
import MultiJdbcFromDatabaseSuggestion from './MultiJdbcFromDatabaseSuggestion.vue';
import MultiJobWareHouseConfig from '@/screens/DataIngestion/components/WareHouseConfig/MultiJobWareHouseConfig.vue';
...
import MultiShopifyFromDatabaseSuggestion from '@/screens/DataIngestion/components/MultiShopifyFromDatabaseSuggestion.vue';
import { ShopifyJob } from '@core/DataIngestion/Domain/Job/ShopifyJob';
import S3DataSourceConfig from './S3Csv/S3DataSourceConfig.vue';
...
export default class MultiJobCreationModal extends Vue {
  ...
  private submitCallback: IngestionJobCallback | null = null;
  private cancelCallback: IngestionJobCallback | null = null;

  @Inject
  private readonly schemaService!: SchemaService;

  @Ref()
  private readonly bigQueryFromDatabaseSuggestion!: MultiBigQueryFromDatabaseSuggestion;

  @Ref()
  private readonly jdbcFromDatabaseSuggestion!: MultiJdbcFromDatabaseSuggestion;

  @Ref()
  private readonly genericJdbcFromDatabaseSuggestion!: MultiGenericJdbcFromDatabaseSuggestion;

  @Ref()
  private readonly shopifyFromDatabaseSuggestion!: MultiShopifyFromDatabaseSuggestion;
  ...
  private get actionName() {
    if (Job.havePreviewSchemaStep(this.jdbcJob)) {
      return 'Preview';
    } else {
  ...
  private get isInvalidDestinationDatabase() {
    return this.isEnableSyncToDataWarehouse && StringUtils.isEmpty(this.jdbcJob.destDatabaseName);
  }
  ...
  async show(jdbcJob: Job, submitCallback: IngestionJobCallback, cancelCallback: IngestionJobCallback) {
    ...
    if (Job.getJobFormConfigMode(this.jdbcJob) === FormMode.Edit) {
      await this.$nextTick(async () => {
        try {
          ...
          Log.debug('JobCreationModal::loadFromData::error::', e.message);
        } finally {
  ...
  private async submitJob(job: Job) {
    ...
    Log.debug('Submit Job', clonedJob);
    switch (jobConfigFormMode) {
      case FormMode.Create:
        if (this.isSingleTable) {
          ...
          await JobModule.createMulti({ job: clonedJob, tables: tableNames });
        }
        break;
      case FormMode.Edit:
  ...
  @Track(TrackEvents.DataSourceSelect, {
    source_id: (_: MultiJobCreationModal, args: any) => args[0].source.id,
  ...
  @Track(TrackEvents.SelectDatabase, { database_name: (_: MultiJobCreationModal, args: any) => args[0] })
  ...
  @Track(TrackEvents.SelectTable, { table_name: (_: MultiJobCreationModal, args: any) => args[0] })
