<template>
  <div class="manage-policy">
    <StatusWidget :status="status" :error="errorMessage" @retry="handleLoadRLSPolicies(true)">
...
import { ViewMode } from '@/screens/data-management/views/data-schema/model';
import { Modals } from '@/utils/Modals';
import { RLSFilterBuilderConfig } from '@/screens/dashboard-detail/components/data-builder-modal/ChartBuilderConfig';
...
export default class ManageRLSPolicy extends Vue {
  ...
  private getOperatorDisplayName(operator: UserAttributeOperator) {
    switch (operator) {
      case UserAttributeOperator.Contain:
        return 'in';
      case UserAttributeOperator.NotContain:
        return 'not in';
      case UserAttributeOperator.Equal:
        return 'equal';
      case UserAttributeOperator.NotEqual:
        return 'not equal';
      case UserAttributeOperator.IsNotNull:
        return 'is not null';
      case UserAttributeOperator.IsNull:
        return 'is null';
    }
  }
  ...
  private async showFilterBuilderModal(policyIndex: number) {
    ...
    this.chartBuilderComponent.showModal({
      ...
      database: new DatabaseInfo(this.tableSchema.dbName, this.tableSchema.organizationId, this.tableSchema.dbName, [this.tableSchema]),
