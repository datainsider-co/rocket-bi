...
@Module({ store: store, namespaced: true, name: Stores.DrilldownStore, dynamic: true })
class DrilldownDataStore extends VuexModule {
  ...
  private databaseAsMap: Map<string, DatabaseInfo> = new Map<string, DatabaseInfo>();
  ...
  reset() {
    ...
    this.databaseAsMap = new Map<string, DatabaseInfo>();
  }
  ...
  @Action
  private async getAdhocDatabaseSchema(sqlViews: InlineSqlView[]): Promise<DatabaseInfo> {
    ...
    return new DatabaseInfo('', -1, 'Ahoc table', listTables);
  }

  @Action
  async getDatabaseSchema(dbName: string): Promise<DatabaseInfo> {
    Log.debug('getDatabaseSchema::', dbName);
    if (this.databaseAsMap.has(dbName)) {
      return this.databaseAsMap.get(dbName)!;
    } else {
      const dbSchema: DatabaseInfo = await DatabaseSchemaModule.fetchDatabaseInfo(dbName);
