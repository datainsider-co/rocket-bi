...
import { LLConnection } from '@/screens/dashboard-detail/components/relationship/RelationshipEditor';
...
export default class TableSchemaItem extends DiagramItem {
  ...
  @Inject('getColumnId')
  private getColumnId!: (dbName: string, tblName: string, colName: string) => string;
  ...
  protected draw(): Konva.Group {
    ...
    this.columns.forEach((col, index) => {
      const rectColumn = new Konva.Group({
        id: this.getColumnId(this.tableSchema.dbName, this.tableSchema.name, col.name),
        x: 0,
        y: rowHeight * (index + 2),
        width,
        height: rowHeight
