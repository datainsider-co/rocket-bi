...
export class SeriesQuerySetting extends QuerySetting<SeriesChartOption> implements Zoomable, Drilldownable, Filterable {
  ...
  setZoomData(data: ZoomData): void {
    if (data.horizontalLevel?.scalarFunction) {
      ...
      Log.debug(
        'setZoomData::filter::',
        this.sorts.filter(sort => isEqual(sort.function.field, data.horizontalLevel.field))
      );
