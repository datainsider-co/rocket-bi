...
import datainsider.jobworker.domain.job.{TikTokAdsJob, TikTokReport}
...
class TikTokReportReader(
  ...
  private def getTikTokAdsData(
    ...
    try {
      val response: TikTokResponse[TikTokAdsData] =
        client.get[TikTokResponse[TikTokAdsData]](endPoint = job.tikTokEndPoint, params = params)
  ...
  private def loadNextTimeRange(nextIndex: Int): Unit = {
    if (nextIndex < timeRanges.length) {
      ...
      throw new AlreadyCompletedException("no more time range to load")
    }
  }
  ...
  /**
    * @throws AlreadyCompletedException disable to continue reading
    */
  override def next(columns: Seq[Column]): Record = {
    loadNext()
    val record = columns.map(column => { getRecordCell(tikTokAdsData.list(curRow), column) })
    curRow = curRow + 1
    record
  }
