...
import com.ptl.util.JsonUtil.JsonNodeLike
...
import datainsider.jobworker.repository.{CoinMarketCapClientImpl, CoinMarketCapListResponse}
import datainsider.jobworker.service.worker.CoinMarketCapWorker.COLUMN_NAME_TO_PATH_MAP
import datainsider.jobworker.util.Implicits.FutureEnhance
import datainsider.jobworker.util.{StringUtils, ZConfig}
...
class CoinMarketCapWorker(
  ...
  private def sync(job: CoinMarketCapJob, syncId: SyncId, onProgress: JobProgress => Future[Unit]): JobProgress = {
    ...
    val tableSchema: TableSchema = CoinMarketCapWorker.getCoinTableSchema(job.orgId, job.destDatabaseName, job.destTableName)
    lazy val writers: Seq[DataWriter] = job.destinations.map(dest => DataWriter(dest))
    ...
    while (hasNext && isRunning.get()) {
      ...
      val records: Seq[Record] = CoinMarketCapWorker.toRecords(tableSchema.columns, response.data, COLUMN_NAME_TO_PATH_MAP)
      if (records.nonEmpty) {
  ...
  private def ensureTableSchema(tableSchema: TableSchema, destinations: Seq[DataDestination]): Unit = {
    info(s"ensureTableSchema: $tableSchema")
