...
import datainsider.client.domain.schema.column.{BoolColumn, DateTimeColumn, Int32Column, StringColumn}
import datainsider.jobworker.domain.{AmazonS3Job, AmazonS3Source, CsvConfig, DataDestination, JobStatus}
import datainsider.jobworker.service.handler.{AmazonS3MetadataHandler, SourceMetadataHandler}
...
class AmazonS3HandlerTest extends FunSuite with BeforeAndAfterAll {
  ...
  override def beforeAll(): Unit = {
    ...
    client.putObject(bucketName, "products.csv", new File("./data/products.csv"))
    super.beforeAll()
  }
