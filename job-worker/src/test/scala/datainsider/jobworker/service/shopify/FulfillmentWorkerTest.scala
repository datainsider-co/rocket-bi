...
class FulfillmentWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "order_fulfilment_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Fulfillment,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "order_fulfilment_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Fulfillment,
  ...
  test("Incremental sync fulfillment test") {
  ...
  test("Incremental sync fulfillment test from latest id") {
  ...
  test("Incremental sync fulfillment test from order_id 4776703262947") {
