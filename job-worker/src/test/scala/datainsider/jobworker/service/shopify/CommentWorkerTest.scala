...
class CommentWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "table_comment_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Comment,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "table_comment_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Comment,
    lastSyncedValue = "129837400291"
  )
  ...
  test("Incremental sync comment from latest id") {
  ...
  test("Incremental sync comment from 129837400291") {
