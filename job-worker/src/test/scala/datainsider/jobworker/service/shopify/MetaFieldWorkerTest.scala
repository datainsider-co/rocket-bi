...
class MetaFieldWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "meta_field_table_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.MetaField,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "meta_field_table_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.MetaField,
    lastSyncedValue = "22697307767011"
  )
  ...
  test("Incremental sync metafield") {
  ...
  test("Incremental sync metafield from latest id") {
  ...
  test("Incremental sync metafield from 69243011299") {
