...
class PriceRuleWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "price_rule_table_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.PriceRule,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "price_rule_table_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.PriceRule,
  ...
  test("Incremental sync price rule") {
  ...
  test("Incremental sync price rule from latest id") {
  ...
  test("Incremental sync price rule from 1180059074787") {
