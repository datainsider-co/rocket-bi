...
class ProductImageWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "product_image_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.ProductImage,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "product_image_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.ProductImage,
  ...
  test("Incremental sync product image") {
  ...
  test("Incremental sync product image from latest id") {
  ...
  test("Incremental sync product image from product id 7690517446883") {
