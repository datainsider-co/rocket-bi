...
class SmartCollectionWorkerTest extends ShopifyWorkerTest {
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "order2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Order,
    lastSyncedValue = "403446923491"
  )
  ...
  test("Incremental sync smart collection test") {
  ...
  test("Incremental sync smart collection test from latest id") {
  ...
  test("Incremental sync smart collection test from 403446923491") {
