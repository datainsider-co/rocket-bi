...
class CustomCollectionWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "custom_collection_table_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.CustomCollection,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "custom_collection_table_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.CustomCollection,
    lastSyncedValue = "403102367971"
  )
  ...
  test("Incremental sync custom collection test") {
  ...
  test("Incremental sync custom collection test from latest id") {
  ...
  test("Incremental sync custom collection test from 403102367971") {
