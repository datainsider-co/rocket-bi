...
class OrderRefundWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "order_refund_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Refund,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "order_refund_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Refund,
  ...
  test("Incremental sync order refund test") {
  ...
  test("Incremental sync order refund test from latest id") {
  ...
  test("Incremental sync order refund test from order_id 4776703262947") {
