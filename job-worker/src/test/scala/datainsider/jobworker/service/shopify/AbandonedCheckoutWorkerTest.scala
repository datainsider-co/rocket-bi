...
class AbandonedCheckoutWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "abandoned_checkout_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.AbandonedCheckout,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "abandoned_checkout_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.AbandonedCheckout,
    lastSyncedValue = "32763788493027"
  )
  ...
  test("Incremental sync abandoned checkout test") {
  ...
  test("Incremental sync abandoned checkout test from latest id") {
  ...
  test("Incremental sync abandoned checkout test from id 32763788493027") {
