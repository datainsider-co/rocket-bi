...
class ThemeWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "theme_table_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Theme,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "theme_table_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Theme,
    lastSyncedValue = "87188668643"
  )
  ...
  test("Incremental sync theme from latest id") {
  ...
  test("Incremental sync theme from 87188668643") {
