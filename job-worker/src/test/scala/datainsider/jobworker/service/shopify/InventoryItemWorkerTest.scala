...
class InventoryItemWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "inventory_item_table_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.InventoryItem,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "inventory_item_table_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.InventoryItem,
  ...
  test("Incremental sync inventory item") {
  ...
  test("Incremental sync inventory item from latest id") {
  ...
  test("Incremental sync inventory item from 7690517446883") {
