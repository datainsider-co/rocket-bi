...
class ReportWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "report_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Report,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "report_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Report,
    lastSyncedValue = "2552561891"
  )
...
private val reader = ShopifyReader(source, job.tableName)
  ...
  test("Incremental sync report from latest id") {
  ...
  test("Incremental sync report from order_id 2552561891") {
