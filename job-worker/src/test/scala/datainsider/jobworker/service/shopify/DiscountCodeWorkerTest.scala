...
class DiscountCodeWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "discount_code_table_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.DiscountCode,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "discount_code_table_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.DiscountCode,
    lastSyncedValue = "1180059074787"
  )
  ...
  test("Incremental sync discount code") {
  ...
  test("Incremental sync discount code from latest id") {
  ...
  test("Incremental sync discount code from collect_id 1180059074787") {
