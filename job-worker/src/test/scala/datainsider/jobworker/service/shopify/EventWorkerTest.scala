...
class EventWorkerTest extends ShopifyWorkerTest {
  ...
  val job2 = ShopifyJob(
    ...
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Event,
    lastSyncedValue = "92050177655011"
  )
  ...
  test("Incremental sync event from latest id") {
  ...
  test("Incremental sync event from 92050177655011") {
