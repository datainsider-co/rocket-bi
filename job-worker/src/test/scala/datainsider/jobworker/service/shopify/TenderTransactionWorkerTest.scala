...
class TenderTransactionWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "tender_transaction_table_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.TenderTransaction,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "tender_transaction_table_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.TenderTransaction,
    lastSyncedValue = "3978524328163"
  )
  ...
  test("Incremental sync tender transactions test") {
  ...
  test("Incremental sync tender transactions test from latest id") {
  ...
  test("Incremental sync tender transactions test from 3978524328163") {
