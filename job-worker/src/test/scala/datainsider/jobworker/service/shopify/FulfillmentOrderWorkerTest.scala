...
class FulfillmentOrderWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "order_fulfilment_order_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.FulfillmentOrder,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "order_fulfilment_order_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.FulfillmentOrder,
  ...
  test("Incremental sync fulfillment order test") {
  ...
  test("Incremental sync fulfillment order test from latest id") {
  ...
  test("Incremental sync fulfillment order test from order_id 4776703262947") {
