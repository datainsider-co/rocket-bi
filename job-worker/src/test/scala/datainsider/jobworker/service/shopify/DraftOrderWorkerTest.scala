...
class DraftOrderWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "draft_order_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.DraftOrder,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "draft_order_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.DraftOrder,
    lastSyncedValue = "1086047748323"
  )
  ...
  test("Incremental sync draft order test") {
  ...
  test("Incremental sync draft order test from latest id") {
  ...
  test("Incremental sync draft order test from order_id 1086047748323") {
