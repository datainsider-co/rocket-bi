...
class InventoryLevelWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "inventory_level_table_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.InventoryLevel,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "inventory_level_table_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.InventoryLevel,
  ...
  test("Incremental sync inventory level") {
    ...
    val latestId = getLatestId(job.destDatabaseName, job.destTableName, "location_id")
  ...
  test("Incremental sync inventory level from latest id") {
  ...
  test("Incremental sync inventory level from 69243011299") {
    ...
    val latestId = getLatestId(job2.destDatabaseName, job2.destTableName, "location_id")
    println(s"current latest id: ${finishedProgress.lastSyncedValue}")
    println(s"actual latest id: ${latestId}")
    assert(finishedProgress.lastSyncedValue == String.valueOf(latestId))
