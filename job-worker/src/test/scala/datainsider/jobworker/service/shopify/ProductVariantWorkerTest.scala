...
class ProductVariantWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "product_variant_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.ProductVariant,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "product_variant_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.ProductVariant,
  ...
  test("Incremental sync product variant") {
    ...
    val latestId = getLatestId(job.destDatabaseName, job.destTableName, "product_id")
  ...
  test("Incremental sync product variant from latest id") {
  ...
  test("Incremental sync product variant from product id 7690517446883") {
    ...
    val latestId = getLatestId(job2.destDatabaseName, job2.destTableName, "product_id")
    println(s"current latest id: ${finishedProgress.lastSyncedValue}")
    println(s"actual latest id: ${latestId}")
    assert(finishedProgress.lastSyncedValue == String.valueOf(latestId))
  }
