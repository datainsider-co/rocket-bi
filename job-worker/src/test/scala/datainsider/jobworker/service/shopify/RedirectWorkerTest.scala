...
class RedirectWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "redirect_table_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Redirect,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "redirect_table_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Redirect,
    lastSyncedValue = "370868060387"
  )
  ...
  test("Incremental sync redirect") {
  ...
  test("Incremental sync redirect from latest id") {
  ...
  test("Incremental sync redirect from 370868060387") {
