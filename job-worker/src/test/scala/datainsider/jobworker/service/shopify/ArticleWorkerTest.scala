...
class ArticleWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "blog_table_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Article,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "blog_table_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Article,
    lastSyncedValue = "587570315491"
  )
  ...
  test("Incremental sync article from latest id") {
  ...
  test("Incremental sync article from 87188668643") {
