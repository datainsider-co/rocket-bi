...
class PageWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "page_table_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Page,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "page_table_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Page,
    lastSyncedValue = "97977401571"
  )
  ...
  test("Incremental sync page  from latest id") {
  ...
  test("Incremental sync page  from 97977401571") {
