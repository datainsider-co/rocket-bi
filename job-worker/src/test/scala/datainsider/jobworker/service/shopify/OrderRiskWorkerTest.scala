...
class OrderRiskWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "order_risk_table_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.OrderRisk,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "order_risk_table_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.OrderRisk,
  ...
  test("Incremental sync order risk test") {
    ...
    val latestId = getLatestId(job.destDatabaseName, job.destTableName, "order_id")
    println(s"current latest id: ${finishedProgress.lastSyncedValue}")
    println(s"actual latest id: ${latestId}")
    lastSyncedValue = finishedProgress.lastSyncedValue
  }

  test("Incremental sync order risk test from latest id") {
  ...
  test("Incremental sync order risk test from order_id 4776703262947") {
    ...
    val latestId = getLatestId(job2.destDatabaseName, job2.destTableName, "order_id")
    println(s"current latest id: ${finishedProgress.lastSyncedValue}")
    println(s"actual latest id: ${latestId}")
  }
}
