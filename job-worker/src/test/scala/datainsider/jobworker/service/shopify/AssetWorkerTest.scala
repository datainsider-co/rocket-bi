...
class AssetWorkerTest extends ShopifyWorkerTest {
  ...
  val job = ShopifyJob(
    ...
    destTableName = "asset_table_test_1",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Asset,
    lastSyncedValue = ""
  )
  ...
  val job2 = ShopifyJob(
    ...
    currentSyncStatus = JobStatus.Syncing,
    destDatabaseName = dbName,
    destTableName = "asset_table_test_2",
    destinations = Seq(DataDestination.Clickhouse),
    tableName = ShopifyTable.Asset,
  ...
  test("Incremental sync asset from latest id") {
  ...
  test("Incremental sync asset from 87188668643") {
