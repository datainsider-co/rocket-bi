...
class SystemControllerTest extends FeatureTest {
  ...
  test("Get System Info") {
    val response: Response = server.httpGet("/system/info", andExpect = Status.Ok)
  ...
  test("refresh connection source") {
    val response: Response = server.httpPost("/system/refresh-schema", postBody = "{}", andExpect = Status.Ok)
