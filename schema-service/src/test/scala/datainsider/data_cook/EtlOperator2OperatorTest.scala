...
import datainsider.data_cook.domain.OperatorInfo
import datainsider.data_cook.domain.operator.EtlOperator

class EtlOperator2OperatorTest extends Test {
  test("convert etl Operators to Operators") {
    val json = """
        ...
        |  "email_configuration" : null
        |} ]""".stripMargin
    val etlOperators = JsonParser.fromJson[Seq[EtlOperator]](json).toArray
