...
class EmailServiceTest extends Test {
  ...
  test("send mail to 1 user") {
    val config = new EmailConfiguration(Array("meomeocf98@gmail.com"), fileName = "sales.csv", subject = "Daily Send Sales Records")
  ...
  test("send email with empty content") {
    val config = new EmailConfiguration(Array("meomeocf98@gmail.com"), fileName = "sales.csv", subject = "Daily Send Sales Records", content = Some(""))
    val path = getClass.getClassLoader.getResource("datasets/orders.csv").getPath
    val response = await(emailService.send(config, path, MediaType.Csv))
    assertResult(response != null)(true)
    println(response.body)
  ...
  test("send email to address not exits") {
    val config = EmailConfiguration(Array("tvc12@ohmypet.app"), fileName = "sales.csv", subject = "Daily Send Sales Records")
    val path = getClass.getClassLoader.getResource("datasets/orders.csv").getPath
    val response = await(emailService.send(config, path, MediaType.Csv))
    assertResult(response != null)(true)
    println(response.body)
  ...
  test("send email with attachments path not exits") {
    try {
      val config = EmailConfiguration(Array("meomeocf98@gmail.com"), fileName = "sales.csv", subject = "Daily Send Sales Records")
