...
/**
  ...
  */
class EtlControllerTest extends FeatureTest {
  ...
  test("end preview etl job") {
    ...
    val response: Response =
      server.httpPost("/data_cook/456/end_preview", postBody = JsonParser.toJson(request), andExpect = Status.NotFound)
...
//    assertResult(true)(responseAsMap("data"))
  ...
  test("List Oracle database") {
    ...
    val request = ListThirdPartyDatabaseRequest(OracleJdbcPersistConfiguration(host, port, serviceName, username, password, databaseName = dbname, tableName = "", persistType = PersistentType.Append))
    val response: Response = server.httpPost("/data_cook/third_party/database/list", JsonParser.toJson(request), andExpect = Status.Ok)
  }
  test("List Mysql database") {
    ...
    val request = ListThirdPartyDatabaseRequest(MySQLJdbcPersistConfiguration(host, port, username, password, databaseName = dbname, tableName = "", persistType = PersistentType.Append))
    val response: Response = server.httpPost("/data_cook/third_party/database/list", JsonParser.toJson(request), andExpect = Status.Ok)
  }
  ...
  test("List Oracle Table") {
    ...
    val password: String = ZConfig.getString("data_cook.jdbc_test.oracle.password")
    val dbname: String = ZConfig.getString("data_cook.jdbc_test.oracle.dbname")
    val request = ListThirdPartyTableRequest( OracleJdbcPersistConfiguration(host, port, serviceName, username, password, databaseName = dbname, tableName = "", persistType = PersistentType.Append), dbname)
    val response: Response = server.httpPost("/data_cook/third_party/table/list", JsonParser.toJson(request), andExpect = Status.Ok)
  }
  test("List Mysql Table") {
    ...
    val password: String = ZConfig.getString("data_cook.jdbc_test.mysql.password")
    val dbname: String = ZConfig.getString("data_cook.jdbc_test.mysql.dbname")
    val request = ListThirdPartyTableRequest(MySQLJdbcPersistConfiguration(host, port, username, password, databaseName = dbname, tableName = "", persistType = PersistentType.Append), dbname)
    val response: Response = server.httpPost("/data_cook/third_party/database/list", JsonParser.toJson(request), andExpect = Status.Ok)
  }
