...
class SSLJdbcTest extends Test {
  val keyStoreData = new String(Base64.getEncoder.encode(FileUtils.readFileToByteArray(new File(getClass.getClassLoader.getResource("keys/key.jks").getPath))), StandardCharsets.UTF_8)
  val trustStoreData = new String(Base64.getEncoder.encode(FileUtils.readFileToByteArray(new File(getClass.getClassLoader.getResource("keys/key.jks").getPath))), StandardCharsets.UTF_8)
