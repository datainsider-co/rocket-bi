...
class OracleOperatorTest extends OperatorTest with Logging {
  ...
  test("Persist Append with table existed") {
    ...
    val saveOperator = OraclePersistOperator(2, host, port, serviceName, username, password, dbname, existedTable, PersistentType.Append, extraPropertiesAsJson = Some("""{"serverTimezone": "Asia/Ho_Chi_Minh"}"""))
