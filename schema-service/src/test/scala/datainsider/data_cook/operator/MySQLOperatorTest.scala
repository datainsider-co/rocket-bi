...
import datainsider.data_cook.pipeline.operator.persist.writer.{MsSQLWriter, MySQLWriter}
import datainsider.data_cook.pipeline.operator.persist.{JDBCPersistResult, MySQLPersistOperator}
 ...
 */
class MySQLOperatorTest extends OperatorTest with Logging {
  ...
  private val host: String = ZConfig.getString("data_cook.jdbc_test.mysql.host")
  private val port: Int = ZConfig.getInt("data_cook.jdbc_test.mysql.port")
  private val username: String = ZConfig.getString("data_cook.jdbc_test.mysql.username")
  private val password: String = ZConfig.getString("data_cook.jdbc_test.mysql.password")
  ...
  private def dropTable(dbname: String, tableName: String): Unit = {
    val operator = MySQLPersistOperator(1, host, port, username, password, dbname, tableName, PersistentType.Append)
  ...
  test("Persist Append with table existed") {
    ...
    val saveOperator = MySQLPersistOperator(2, host, port, username, password, dbname, existedTable, PersistentType.Append)
  ...
  test("Persist Append with table not existed") {
    ...
    val saveOperator = MySQLPersistOperator(2, host, port, username, password, dbname, tableNotExistsTable, PersistentType.Append)
  ...
  test("Persist Override with table existed") {
    ...
    val saveOperator = MySQLPersistOperator(2, host, port, username, password, dbname, existedTable, PersistentType.Replace, Some("""{"serverTimezone": "Asia/Ho_Chi_Minh"}"""))
  ...
  test("Persist Override with table not existed") {
    ...
    val rootOperator = RootOperator(0)
    val getOperator = GetOperator(1, orderSchema, TableConfiguration("persist_3", "ETL", "ETL"))
    val saveOperator = MySQLPersistOperator(2, host, port, username, password, dbname, tableNotExistsTable, PersistentType.Replace)
  ...
  test("Persist Override with string column") {
    ...
    val saveOperator = MySQLPersistOperator(3, host, port, username, password, dbname, tableNotExistsTable, PersistentType.Replace)
