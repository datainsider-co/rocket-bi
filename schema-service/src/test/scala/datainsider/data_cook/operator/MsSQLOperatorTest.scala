...
import datainsider.data_cook.pipeline.operator.persist.writer.MsSQLWriter
import datainsider.data_cook.pipeline.operator.persist.{JDBCPersistResult, MsSQLPersistOperator}
...
/**
  ...
  */
class MsSQLOperatorTest extends OperatorTest with Logging {
  override protected val jobId: EtlJobId = 1454
  implicit val executorResolver: ExecutorResolver = injector.instance[ExecutorResolver]

  private val host: String = ZConfig.getString("data_cook.jdbc_test.mssql.host")
  private val port: Int = ZConfig.getInt("data_cook.jdbc_test.mssql.port")
  private val username: String = ZConfig.getString("data_cook.jdbc_test.mssql.username")
  private val password: String = ZConfig.getString("data_cook.jdbc_test.mssql.password")
  private val catalogName: String = ZConfig.getString("data_cook.jdbc_test.mssql.catalog")
  private val databaseName = "dbo_1"
  ...
  private def dropTable(dbname: String, tableName: String): Unit = {
    val operator = MsSQLPersistOperator(
      2,
      host,
      port,
      username,
    ...
    val writer = new MsSQLWriter(client)
  ...
  private def getTotalRows(operator: MsSQLPersistOperator): Long = {
    val client = NativeJdbcClient(operator.jdbcUrl, operator.username, operator.password)
  ...
  test("Persist Append with table existed") {
    ...
    val saveOperator = MsSQLPersistOperator(
      2,
      host,
      port,
      username,
