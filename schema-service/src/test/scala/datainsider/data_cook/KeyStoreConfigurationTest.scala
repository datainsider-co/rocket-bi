...
import datainsider.data_cook.domain.persist.KeyStoreConfiguration
...
class KeyStoreConfigurationTest extends Test {
  test("Decode and and code key file by base64") {
    ...
    val key2AsString = new String(Base64.getEncoder.encode(FileUtils.readFileToByteArray(file)), StandardCharsets.US_ASCII)
