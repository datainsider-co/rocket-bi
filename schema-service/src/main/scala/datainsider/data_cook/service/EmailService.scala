...
case class SendGridEmailService(
  ...
  def ensureFileSize(file: File): Unit = {
    ...
    if (fileLength > limitSizeInBytes) {
      throw InternalError(s"attachments path ${file.getPath} too large, current size is ${fileLength} in bytes but limit is ${limitSizeInBytes} in bytes")
    }
  }
  ...
  @throws[IOException]
  @throws[InternalError]
  private def buildSendgridMail(emailConfig: EmailConfiguration, attachmentPaths: Seq[String], mediaType: String): Mail = {
  ...
  @throws[InternalError]
  override def send(emailConfig: EmailConfiguration, attachmentPath: String, fileType: String): Future[EmailResponse] = {
    Future {
  ...
  @throws[InternalError]
  override def send(emailConfig: EmailConfiguration, attachmentPaths: Seq[String], fileType: String): Future[EmailResponse] = {
    Future {
