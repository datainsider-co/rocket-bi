...
class EtlJobServiceImpl @Inject() (
    ...
    deletedEtlService: TrashEtlJobService,
  ...
  override def softDelete(organizationId: OrganizationId, id: EtlJobId): Future[EtlJobResponse] = {
    for {
      ...
      isOk <- deletedEtlService.add(organizationId, etlJob)
      _ <-
  ...
  override def transfer(organizationId: EtlJobId, fromUsername: UserId, toUsername: UserId): Future[Boolean] = {
    for {
      _ <- etlJobRepository.transferOwner(organizationId, fromUsername, toUsername)
      _ <- deletedEtlService.transferOwner(organizationId, fromUsername, toUsername)
    } yield true
  }

  override def deleteUserData(organizationId: OrganizationId, userId: UserId): Future[Boolean] = {
    for {
      _ <- etlJobRepository.deleteByOwnerId(organizationId, userId)
      _ <- deletedEtlService.deleteByOwnerId(organizationId, userId)
    } yield true
  }

  override def listSharedEtlJobs(organizationId: OrganizationId, request: ListEtlJobsRequest): Future[PageResult[EtlJobResponse]] = {
    for {
      etlJobs <- etlJobRepository.listSharedJobs(organizationId, request)
      jobResponses <- toJobResponses(organizationId, etlJobs._2)
    } yield PageResult(etlJobs._1, jobResponses)
  }
}
...
class MockEtlJobService extends EtlJobService {
   ...
   * transfer owner of etl job
   */
  override def transfer(organizationId: EtlJobId, fromUsername: UserId, toUsername: UserId): Future[Boolean] = Future.True

  override def deleteUserData(organizationId: OrganizationId, userId: UserId): Future[Boolean] = Future.True

  override def listSharedEtlJobs(organizationId: OrganizationId, request: ListEtlJobsRequest): Future[PageResult[EtlJobResponse]] = Future.value(PageResult(0, Seq.empty))
}
