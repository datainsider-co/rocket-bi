...
object DataCookModule extends TwitterModule {
  ...
  def providesPreviewExecutorResolver(
      ...
      @Named("data_cook_clickhouse") client: JdbcClient,
      @Named("preview_python_executor") pythonOperatorExecutor: Executor[PythonOperator]
  ): ExecutorResolver = {
  ...
  @Named("preview_python_executor")
  @Provides
  @Singleton
  def providesPreviewPythonExecutor(
      @Named("preview_table_service") tableService: EtlTableService,
      @Named("etl_schema_service") schemaService: SchemaService
    ...
    val executeTimeout: Long = ZConfig.getLong("data_cook.python_execute_timeout", 60000)
  ...
  def providesPythonExecutor(
    ...
    val password: String = ZConfig.getString("data_cook.clickhouse.password")
    val tmpDir: String = ZConfig.getString("data_cook.tmp_dir")
    val executeTimeout: Long = ZConfig.getLong("data_cook.python_execute_timeout", 7200000)
