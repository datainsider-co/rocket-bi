...
object EtlOperator {
  ...
  def getDestTables(operator: EtlOperator): ArrayBuffer[TblName] = {
    operator match {
      ...
      case operator: PythonOperator => {
        val nestedTables = getDestTables(operator.operator)
        nestedTables.append(operator.destTableConfiguration.tblName)
        nestedTables
      }
      ...
      case operator: SendToGroupEmailOperator => {
        ArrayBuffer(operator.destTableConfiguration.tblName)
      }
    }
  }
