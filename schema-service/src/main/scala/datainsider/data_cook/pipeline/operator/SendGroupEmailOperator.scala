...
case class TestGroupSendEmailExecutor() extends Executor[SendGroupEmailOperator] {
  ...
  private def ensureInput(
    ...
    val parentTables: Seq[OperatorResult] =
      operator.parentIds.map(id => mapResults.get(id)).filter(_.isDefined).map(_.get)
    if (parentTables.size == operator.parentIds.size) {
