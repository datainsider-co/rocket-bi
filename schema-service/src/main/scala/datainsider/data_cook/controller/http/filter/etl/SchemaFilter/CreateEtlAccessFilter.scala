...
case class CreateEtlAccessFilter @Inject() (authorizationService: OrgAuthorizationClientService)
    ...
    if (request.isAuthenticated) {
      authorizationService
        .isPermitted(
          ...
          PermissionProviders.permissionBuilder.perm(organizationId, PermResourceType.ETL.toString, action, "*")
        )
        .map {
          case true  => Permitted()
          case false => UnPermitted("No permission to create this job.")
        }
    } else {
      Future.value(UnPermitted("Login is required to create this job."))
    }
  }
}
