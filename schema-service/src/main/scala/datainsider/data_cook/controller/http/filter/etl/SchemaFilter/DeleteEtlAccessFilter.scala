...
case class DeleteEtlAccessFilter @Inject() (
  ...
  private def isUserPermitted(request: Request): Future[PermissionResult] = {
    ...
    if (request.isAuthenticated) {
      ...
      authorizationService
        ...
        .map {
          case true  => Permitted()
          case false => UnPermitted("No permission to delete this job.")
        }
    } else {
      Future.value(UnPermitted("Login is required to delete this job."))
    }
  }
}
