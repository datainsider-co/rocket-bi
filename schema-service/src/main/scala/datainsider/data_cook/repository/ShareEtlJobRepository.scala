...
import datainsider.ingestion.domain.{PageResult, SharingInfo}
import datainsider.ingestion.repository.ObjectShareRepository.EnhanceImplicitResultSet
...
class ShareEtlJobRepositoryImpl(client: JdbcClient, dbName: String, tblName: String, generator: IDGenerator[Int])
    extends ShareEtlJobRepository {
  ...
  override def softDelete(
    ...
    val data: Array[Seq[Any]] = usernames
      .map(Seq(System.currentTimeMillis(), organizationId, resourceType, resourceId, _))
      .toArray
  ...
  def createInsertShareInfoData(
    ...
    val fn: Seq[Future[Seq[Any]]] = usernames.map(username => {
      generateId().map(id => {
        Seq(
          id,
          organizationId,
  ...
  override def updateUpdatedTimeShareInfo(shareIds: Seq[String]): Future[Boolean] = {
    val shareData: Seq[Seq[Any]] = shareIds.map(shareId => Seq(System.currentTimeMillis(), shareId))
    val r: Boolean = client.executeBatchUpdate(
      s"""
         |UPDATE $getTable
         |SET updated_at = ?
