...
class EtlJobRepositoryImpl(client: JdbcClient, dbName: String, tblName: String) extends EtlJobRepository {
  import EtlJob.EnhanceImplicitResultSet

  val shareTblName: String = ZConfig.getString("data_cook.mysql.share_table", "share_info")
    ...
    Future {
      ...
      val query =
        s"""
         ...
         |or id in (select resource_id from $dbName.$shareTblName WHERE is_deleted = FALSE AND organization_id = ? AND 	resource_type = ? AND username  = ?)
         |$orderByStatement
         |limit ? offset ?
         |""".stripMargin

      val data = client.executeQuery(
        query,
        s"%${request.keyword}%",
        request.currentUsername,
        organizationId,
    ...
    Future {
      val query =
        s"""
         ...
         |and job_status != 'Syncing'
         |order by next_execute_time ASC;
         |""".stripMargin

      client.executeQuery(query)(toEtlJobs).headOption
    }
   ...
   */
  override def list(from: Long, size: Long): Future[Seq[EtlJob]] = Future {
    val query =
      s"""
         |select * from $dbName.$tblName
         |limit ? offset ?
  ...
  override def migrateData(jobs: Seq[EtlJob]): Future[Unit] = Future{
    val query =
      s"""
         |update $dbName.$tblName
    ...
    val records: Seq[Seq[Any]] = jobs.map(job => Seq(
      JsonParser.toJson(job.operatorInfo, false),
      JsonParser.toJson(job.config, false),
      job.id,
      job.organizationId
