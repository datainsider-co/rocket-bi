...
import datainsider.ingestion.domain.{RefreshBy, RefreshStatus, SystemInfo}
import datainsider.ingestion.repository.{ClickhouseMetaDataHandler, ClickhouseMetaDataHandlerImpl, SchemaMetadataStorage}
...
class RefreshSchemaWorkerImpl @Inject()(
    ...
    override def run(): Unit = {
      while (isRunning.get()) {
        try {
          ...
          val setRefreshStatus = (status: RefreshStatus, errorMsg: Option[String]) => systemService.updateSystemInfo(systemInfo.orgId, status, errorMsg, RefreshBy.System)
  ...
  private def refreshSchema(systemInfo: SystemInfo, setRefreshStatus: (RefreshStatus, Option[String]) => Future[Unit]): Unit = {
    try {
  ...
  /**
    ...
    */
  def refreshSchema(orgId: Long, refreshBy: RefreshBy): Future[Boolean] = {
    systemService.getSystemInfo(orgId).map(systemInfo => {
      ...
      if (isRefreshSchema(systemInfo, refreshBy)) {
        val thread = new Thread(() => {
          ...
          val setRefreshStatus = (status: RefreshStatus, errorMsg: Option[String]) => systemService.updateSystemInfo(systemInfo.orgId, status, errorMsg, refreshBy)
          refreshSchema(systemInfo, setRefreshStatus)
