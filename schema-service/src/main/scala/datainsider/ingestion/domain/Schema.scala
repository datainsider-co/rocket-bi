...
import datainsider.client.domain.query.{Field, Limit, SqlQuery, TableView, ViewField}
import datainsider.client.exception.TableNotFoundError
import datainsider.client.util.JsonParser
import datainsider.ingestion.domain
import datainsider.ingestion.domain.TableStatus.TableStatus
...
object TableType extends Enumeration {
  ...
  val View: domain.TableType.Value = Value("view")
  val Materialized: domain.TableType.Value = Value("materialized")
  val Default: domain.TableType.Value = Value("default")
  val InMemory: domain.TableType.Value = Value("in_memory")
  // serve for etl view, only create etl on one node, high performance
  val EtlView: domain.TableType.Value = Value("etl_view")
  // table with logic replace data when two row same sort key value
  val Replacing: domain.TableType.Value = Value("replacing")
}
