package datainsider.ingestion.controller.http.filter.SchemaFilter
    ...
    if (request.isAuthenticated) {
      ...
      authorizationService
        .isPermitted(
          ...
          PermissionProviders.database.withOrganizationId(organizationId).withDbName(dbName).delete()
        )
        .map {
          case true  => Permitted()
      ...
      Future.value(UnPermitted("Login is required to delete this database."))
    }
  }
}
