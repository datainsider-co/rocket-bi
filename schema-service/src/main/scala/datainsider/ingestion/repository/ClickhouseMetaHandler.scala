...
class ClickhouseMetaDataHandlerImpl(client: JdbcClient) extends ClickhouseMetaDataHandler with Logging {
  ...
  override def getDatabaseNames(): Seq[String] = Profiler("[ClickhouseMetaDataHandlerImpl].getDatabaseNames") {
    try {
  ...
  override def getTables(organizationId: Long, dbName: String, ignoredEngines: Seq[String]): Seq[TableSchema] = Profiler("[ClickhouseMetaDataHandlerImpl].getTables"){
    try {
  ...
  override def getColumns(dbName: String, tblName: String): Seq[Column] = Profiler("[ClickhouseMetaDataHandlerImpl].getColumns") {
    try {
      val columnsQuery = """
                           |SELECT *
                           |FROM system.columns
  ...
  private def toColumns(rs: ResultSet): Seq[Column] = {
    ...
    while (rs.next()) {
      ...
      val column: Column = `type` match {
        case _ if (`type`.contains("UInt8"))  => UInt8Column(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
        case _ if (`type`.contains("UInt16")) => UInt16Column(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
        case _ if (`type`.contains("UInt32")) => UInt32Column(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
        case _ if (`type`.contains("UInt64")) => UInt64Column(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
        // do not move Int to up because it will be matched with UInt8, UInt16, UInt32, UInt64
        case _ if (`type`.contains("Int8"))   => Int8Column(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
        case _ if (`type`.contains("Int16"))  => Int16Column(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
        case _ if (`type`.contains("Int32"))  => Int32Column(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
        case _ if (`type`.contains("Int64"))  => Int64Column(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
        case _ if (`type`.contains("Float"))  => FloatColumn(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
        case _ if (`type`.contains("Float64"))    => DoubleColumn(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
        case _ if (`type`.contains("DateTime"))   => DateTimeColumn(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
        // force datetime64 to date time column, cause system work as well
        case _ if (`type`.contains("DateTime64")) => DateTimeColumn(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
        // do not move up or down this case, because it is a special case
        case _ if (`type`.contains("Date"))   =>  DateColumn(name, displayName = name, isNullable = isNullable, defaultExpression = defaultExpression)
