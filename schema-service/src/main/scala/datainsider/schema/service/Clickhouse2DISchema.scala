...
import datainsider.schema.repository.{ClickhouseMetaDataHandler, SchemaMetadataStorage}
...
class Clickhouse2DISchema(
  ...
  private def dropUnknownDatabase(organizationId: Long, clickhouseDbNames: Seq[String]): Unit = {
    try {
      ...
      val result: Seq[Future[Boolean]] =
        unknownDatabases.map(dbName => storage.deleteDatabaseSchema(organizationId, dbName))
