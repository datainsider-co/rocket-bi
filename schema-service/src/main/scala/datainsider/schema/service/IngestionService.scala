...
import datainsider.schema.controller.http.requests.{ClearTableRequest, EnsureSparkSchemaRequest, IngestFakeDataRequest, IngestRequest}
import datainsider.schema.controller.http.responses.IngestResponse
import datainsider.schema.domain.Types.{DBName, TblName}
import datainsider.schema.domain.{SparkWriteMode, TableSchema}
import datainsider.schema.misc.ColumnDetector
import datainsider.schema.misc.JdbcClient.Record
import datainsider.schema.misc.parser.ClickHouseDataParser
import datainsider.schema.misc.parser.DataParser.Result
import datainsider.schema.repository.DataRepository
...
case class IngestionServiceImpl @Inject() (
  ...
  override def ingestArrayRecords(
    ...
    for {
      ...
      response <- dataRepository.writeRecords(tableSchema, records, 1000)
    } yield response
  }
...
//    })
  ...
  private def buildIngestResponse(result: Result, totalInsertedRecords: Long) = {
    IngestResponse(
