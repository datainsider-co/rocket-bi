<template>
  <div>
    ...
    <template v-if="pickerType === PickerType.InputAndPreview">
      <BInputGroup>
        <BInput :id="genInputId(id)" type="text" aria-label="Input Color" v-model="currentColorAsHex"></BInput>
        <BInputGroupAppend>
          <div
            :id="id"
            tabindex="0"
            class="d-flex flex-row btn-color-picker justify-content-center align-items-center cursor-pointer"
            @click.prevent="toggleShowPicker"
          >
    ...
    <template v-if="pickerType === PickerType.OnlyPreview">
      <div :id="id" tabindex="0" class="picker-preview" @click.prevent="toggleShowPicker">
