    ...
    <div class="container-fluid h-100 w-100">
      <div class="header-content" :class="{ open: showedMenu }">
        ...
        <div v-if="isLogin" class="menu">
          ...
          <!--          </div>-->
          <router-link v-if="isShowDateLeft" v-slot="{ href, navigate }" active-class="item-selected" class="header-bar-item" :to="planAndBillingRoute">
            <a :id="genBtnId('dayleft')" :class="{ active: false }" :href="href" :title="dayLeft" class="navigator" @click="navigate">
              <span>{{ dayLeft }}</span>
            </a>
          </router-link>
...
export default class HeaderBar extends Vue {
  ...
  private planAndBillingRoute = { name: Routers.PlanAndBilling };
  ...
  private get isShowDateLeft() {
    return PlanAndBillingModule.planDetail && PlanAndBillingModule.planDetail.product.name === PlanType.Trial;
  }

  private get dayLeft() {
    const plan = PlanAndBillingModule.planDetail;
    if (plan) {
      const timeLeft = DateUtils.timeLeft(plan.subscription.endTime);
      const planName = PlanDisplayNames[plan.product.name as PlanType];
      return timeLeft + ' ' + planName;
    } else {
      return '';
    }
  }
  ...
  private get isEnableLake(): boolean {
    return !EnvUtils.isDisableLakeHouse();
  }

  private get isEnableIngestion(): boolean {
    return !EnvUtils.isDisableIngestion();
  }

  private get isEnableCDP(): boolean {
    return !EnvUtils.isDisableCDP();
  }
