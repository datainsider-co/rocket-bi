...
export class BubblePromptGenerator extends ChartPromptGenerator {
  generate(_: ShortInfoTableSchema): string {
    return `
    Task:
    - Identify the xAxis and functions required for a bubble chart..
    - Identify the yAxis and functions required for a bubble chart..
    - Identify the value and functions required for a bubble chart..
    - Identify the legend (optional) and functions for a bubble chart..
    - Optional: Specify sorting based on scatter config.
    Output Format (JSON):
    + "xAxis", "yAxis","value", "legendOptional": An array of objects, each with:
    ...
    - "displayName", "field", "functionFamily", "functionType": similar "xAxis", "yAxis","value", "legendOptional".
    ${this.sortingPrompt()}
    `;
  }
