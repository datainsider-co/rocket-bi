<template>
  <div v-if="enable" class="table-chart-pagination-content">
    <div class="footer-total">
      <span v-if="!hideTotal && isShowEntries">{{ totalRows }} {{ totalRowTitle }}</span>
    </div>
...
export default class PaginationComponent extends Vue {
  ...
  static readonly MAX_LIMIT_ITEM = 3;
  static readonly MIN_LIMIT_ITEM = 1;
  @Ref()
  ...
  private get limitItem(): number {
    return this.isShowEntries ? PaginationComponent.MAX_LIMIT_ITEM : PaginationComponent.MIN_LIMIT_ITEM;
  }
}
</script>
