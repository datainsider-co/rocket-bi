...
export default class ParliamentChart extends BaseHighChartWidget<SeriesTwoResponse, ParliamentChartOption, ParliamentQuerySetting> {
  ...
  constructor() {
    ...
    this.numberFormatter = this.buildFormatterByMetricNumber(
      this.setting.options.plotOptions?.item?.dataLabels?.displayUnit ?? MetricNumberMode.Default,
  ...
  protected buildFormatterByMetricNumber(metricNumber: MetricNumberMode, precision: number, decimalPoint?: string, thousandSep?: string) {
