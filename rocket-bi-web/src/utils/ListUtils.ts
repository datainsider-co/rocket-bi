...
export abstract class ListUtils {
  ...
  static group<T>(list: T[], chunkSize: number): T[][] {
    const results: T[][] = [];
  ...
  /**
   * replace element at index, if index is out of range, will insert at end of list
   * @param list
   * @param index
   * @param newData
   */
  static replaceAt<T>(list: T[], index: number, newData: T): T[] {
    if (index > list.length - 1) {
      list.push(newData);
      return list;
    } else {
