<template>
  <div>
    ...
    <DiInputComponent
      ...
      @enter="$emit('enter')"
    />
    <DiInputDateTime id="param-date-input" v-if="type === types.date" :value.sync="syncValue" />
    <template v-if="type === types.list">
      <div>
        <label>Default select</label>

        <DiDropdown id="select-param-list" placeholder="Select default value..." :data="listAsOptions" v-model="syncValue" valueProps="id" labelProps="name" />
      </div>
      <div class="mt-3">
