...
import AbstractSchemaComponent from '@/screens/data-management/views/AbstractSchemaComponent';
...
export default class MeasureFieldManagement extends Vue {
  ...
  private async loadSchema(tableSchema: TableSchema, force = true): Promise<void> {
    try {
  ...
  private async deleteMeasureField(tableSchema: TableSchema, column: Column) {
    try {
      ...
      this.updateRenderData(deletingFieldData.dbName, tableSchema.name);
    } catch (ex) {
