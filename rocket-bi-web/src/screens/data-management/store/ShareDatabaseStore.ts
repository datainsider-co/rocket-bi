...
class ShareDatabaseStore extends VuexModule {
  ...
  @Action
  updatePermissionWithSharedUser(payload: { userData: SharedUserInfo; editedValue: ActionType }) {
    Log.debug('ShareDatabaseModule::updatePermissionWithSharedUser::payload::', payload);
    const { userData, editedValue } = payload;
    switch (editedValue) {
      case ActionType.none:
        ...
        break;
      default:
        this.setEditedUser({ shareId: userData.id, actions: ActionTypeMapActions[editedValue] });
    }
  }
  ...
  @Action
  updatePermissionWithNotSharedUser(payload: { userData: SharedUserInfo; editedValue: ActionType }) {
    Log.debug('ShareDatabaseModule::updatePermissionWithNotSharedUser::payload::', payload);
    switch (payload.editedValue) {
      case ActionType.none:
        this.deleteCreatedUser(payload.userData.user.username);
        break;
      default:
        this.setCreatedUser({ username: payload.userData.user.username, actions: ActionTypeMapActions[payload.editedValue] });
    }
  }
