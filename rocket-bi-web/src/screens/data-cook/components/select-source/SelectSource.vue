<template>
  <div class="dropdown-menu-events text-left">
    <div v-if="!hideHeader" class="font-weight-bold mb-2">
      ...
      <span v-if="!loading">({{ finalDatabaseInfos.length }})</span>
    </div>
    <div class="input-group mb-2">
      ...
      <input ref="keywordInput" v-model.trim="keyword" type="text" debounce="100" placeholder="Search databases & tables..." class="form-control" />
    </div>
    <vuescroll>
      <LoadingComponent v-if="loading" style="height: 250px"></LoadingComponent>
      <div v-else style="max-height: 250px">
        <ErrorWidget v-if="errorMsg" :error="errorMsg" @onRetry="initDatabases"></ErrorWidget>
        <span v-else-if="!finalDatabaseInfos.length" class="list-events-item text-muted font-italic">
          No database
        </span>
        <template v-else>
          ...
          <ul v-for="db in finalDatabaseInfos" :key="db.name" class="list-events">
            <li class="list-events-title">
