...
export default class ManageEtlOperator extends Vue {
  ...
  private set leavesOperators(value: EtlOperator[]) {
    if (this.value) {
  ...
  private async initModel() {
    try {
      ...
      Log.info('initModel::operators', this.leavesOperators.length);
      await this.loadEtlDatabase();
      // Init All Get / NotGet Operators
      this.leavesOperators.forEach(operator => {
      ...
      this.multiPreviewOperators(this.leavesOperators, true);
      if (ListUtils.isNotEmpty(this.allOperators)) {
        this.selectOperator(ListUtils.getHead(this.allOperators)!, false);
      }
    } catch (ex) {
  ...
  private handleMergeEmailConfig(sourceOperator: EtlOperator, position: DragPosition) {
    if (this.currentEmailConfigOverlap) {
      ...
      this.sendToEmail.showEditModal(finalEmailOperator, (newOperator: SendToGroupEmailOperator) => {
        ...
        this.handleAddNewOperator(ListUtils.getHead(newOperator.operators) || null, newOperator);
      });
  ...
  private selectOperator(operator: EtlOperator, isClearServerCache: boolean): void {
    if (this.selectedOperator?.destTableName !== operator.destTableName) {
      this.selectedOperator = operator;
      this.previewOperator(operator, isClearServerCache);
    }
  }
   ...
   */
  private async multiPreviewOperators(operators: EtlOperator[], isClearServerCache: boolean): Promise<void> {
    if (ListUtils.isNotEmpty(operators)) {
      ...
      try {
        ...
        const previewData: PreviewEtlResponse = await this.dataCookService.multiPreview(this.value.id, operators, isClearServerCache);
  ...
  private renderError(previewData: PreviewEtlResponse): void {
    previewData.errors.forEach(errorData => {
      ...
      const rootOperatorError: EtlOperator | undefined = this.allOperators.find(operator => operator.destTableName === tableNameError);
      const relativeOperatorErrors: EtlOperator[] = this.leavesOperators.flatMap(operator => {
   ...
   * preview a operator
   */
  private async previewOperator(operator: EtlOperator, isClearServerCache: boolean): Promise<void> {
    ...
    if (isClearServerCache) {
      await this.multiPreviewOperators(this.leavesOperators, true);
    } else if (isEmptyData && !isPreviewing) {
      let operators: EtlOperator[] = this.leavesOperators.filter(leafOperator => leafOperator.isIncludeOperator(operator.destTableName));
      if (ListUtils.isEmpty(operators)) {
        operators = [operator];
      }
      await this.multiPreviewOperators(operators, isClearServerCache);
    }
  }
  ...
  private startQueryOperator(operator: EtlOperator): void {
    ...
    if (this.queryTable && tableSchema) {
      ...
      this.queryTable.add(this.value.id, operator, tableSchema, newOperator => {
        this.handleAddNewOperator(operator, newOperator);
  ...
  private editQueryOperator(operator: QueryOperator): void {
    ...
    this.queryTable.edit(this.value.id, operator, tableSchema, (newOperator: QueryOperator) => {
      if (operator.query !== newOperator.query) {
        this.processUpdateOperator<QueryOperator>(newOperator.destTableName, ope => {
  ...
  private browseDeathAndAliveNodes(root: EtlOperator, deletedNode: EtlOperator): [EtlOperator[], EtlOperator[]] {
    ...
    while (current) {
      ...
      const leftOperators: EtlOperator[] = current.getParentOperators();
  ...
  private handleRemoveOperator(deletedOperator: EtlOperator) {
    const newOperators = cloneDeep(this.leavesOperators);
    this.leavesOperators.forEach((rootOperator, index) => {
      const [deathOperators, aliveOperators] = this.browseDeathAndAliveNodes(rootOperator, deletedOperator);
      deathOperators.forEach(this.removeOperator);
      // re-calculated index
      newOperators.splice(index, 1, ...aliveOperators);
    });
    Log.debug('handleRemoveOperator::operators::size', this.leavesOperators.length);
    this.leavesOperators = this.compactOperators(newOperators);
    Log.debug('handleRemoveOperator::afterCompact:operators::size', this.leavesOperators.length);
    // if (this.selectedOperator &&)
    // select current operator if it's not deleted
    const isExist = this.allOperators.find(operator => operator.destTableName === this.selectedOperator?.destTableName);
    if (!isExist && ListUtils.isNotEmpty(this.allOperators)) {
      this.selectOperator(ListUtils.getHead(this.allOperators)!, false);
    }
  }
  ...
  private processUpdateOperator<T extends EtlOperator>(destTableName: string, processUpdate: (operator: T) => void) {
    this.leavesOperators.forEach(rootOperator => {
  ...
  private startJoinOperatorFromTable(operator: EtlOperator, database: DatabaseSchema, table: TableSchema): void {
   ...
   */
  private handleAddNewOperator(parentOperator: EtlOperator | null, newOperator: EtlOperator | null) {
    if (newOperator) {
      Log.debug('handleNewOperator::', newOperator?.destTableName);
      const parentName = parentOperator?.destTableName;
      const currentLeafIndex: number = this.leavesOperators.findIndex(operator => operator.destTableName === parentName);
      ...
      if (currentLeafIndex >= 0) {
        ...
        this.leavesOperators[currentLeafIndex] = newOperator;
        this.leavesOperators = this.leavesOperators.concat([]);
      } else {
        this.leavesOperators = this.leavesOperators.concat(newOperator);
      }
      Log.debug('handleNewOperator::leavesOperators:size', this.leavesOperators.length);

      this.leavesOperators = this.compactOperators(this.leavesOperators);
      Log.debug('handleNewOperator::afterCompact::leavesOperators:size', this.leavesOperators.length);
  ...
  private handleAfterAddOperator(newOperator: EtlOperator) {
    ...
    if (!newOperator.isSendToGroupEmail) {
      this.selectOperator(newOperator, false);
    }
  }
  ...
  private save() {
    ...
    const request = new EtlJobRequest(this.value.displayName, this.leavesOperators, this.value.scheduleTime, this.getETLExtraData(), this.value.config);
  ...
  private onSelectSource(database: DatabaseSchema, table: TableSchema) {
    if (!this.model.mapTable[table.tableIdAddress]) {
      ...
      this.handleAddNewOperator(null, newOperator);
    }
  }
  ...
  private onOperatorsChanged() {
    if (this.leavesOperators.length <= 0) {
      this.model = new ManageEtlModel();
      this.maxTop = MIN_TOP;
    }
  }
