...
import { ChartInfo, Field, FieldRelatedFunction, GeoArea, Group, MapQuerySetting, WidgetId } from '@core/common/domain/model';
...
import { DashboardControllerModule, DrilldownDataStoreModule, FilterModule, QuerySettingModule } from '@/screens/dashboard-detail/stores';
...
export default class DrilldownSetting extends Vue {
  ...
  private async handleLoadDrilldownValues(): Promise<void> {
    ...
    const newData: AbstractTableResponse | void = await DrilldownDataStoreModule.loadDrilldownValues({
      ...
      filterRequests: FilterModule.getFilters(this.widgetId),
  ...
  private drilldown(fieldDetailInfo: FieldDetailInfo, toFieldRelatedFn: FieldRelatedFunction): void {
    if (Drilldownable.isDrilldownable(this.currentQuery)) {
      ...
      DashboardControllerModule.renderChart({ id: this.metaData.id });
    }
  }
