<template>
  <LayoutContent>
    ...
    <!--    <ClickhouseSourceModal ref="configModal"></ClickhouseSourceModal>-->
    <!--    <ClickhouseSetupProgressModal ref="setupProgressModal"></ClickhouseSetupProgressModal>-->
    <SourceSelectionModal ref="sourceSelectionModal" />
    <DataSourceConfigModal ref="sourceConfigModal" />
    <DataSourceSetupProgressModal ref="progressModal" />
  </LayoutContent>
</template>
<script lang="ts">
...
import { DateTimeFormatter, ListUtils, PopupUtils } from '@/utils';
...
import ClickhouseSetupProgressModal from '@/screens/organization-settings/views/clickhouse-config/ClickhouseSetupProgressModal.vue';
import { ConnectionModule } from '../../stores/ConnectionStore';
import { DataSource, DataSourceType, RefreshSchemaHistory } from '@core/clickhouse-config';
import SourceSelectionModal from '@/screens/organization-settings/views/source-selection/SourceSelectionModal.vue';
import DataSourceConfigModal from '@/screens/organization-settings/views/datasource-config/DataSourceConfigModal.vue';
import DataSourceSetupProgressModal from '@/screens/organization-settings/views/datasource-config/DataSourceSetupProgressModal.vue';
...
} from '@/screens/organization-settings/views/clickhouse-config/header_builder';
...
@Component({
  components: {
    ...
    StatusWidget,
    DiTable2,
    ClickhouseSetupProgressModal,
    SourceSelectionModal,
    DataSourceConfigModal,
    DataSourceSetupProgressModal
  }
})
export default class ClickhouseConfig extends Vue {
  ...
  @Ref()
  private configModal!: ClickhouseSourceModal;

  // @Ref()
  // private setupProgressModal!: ClickhouseSetupProgressModal;

  @Ref()
  private progressModal!: DataSourceSetupProgressModal;

  @Ref()
  private sourceSelectionModal!: SourceSelectionModal;

  @Ref()
  private sourceConfigModal!: DataSourceConfigModal;
  ...
  private get headers(): HeaderData[] {
    ...
    return [
      ...
      },
      {
        ...
        width: 200,
        customRenderBodyCell: new CustomCell(rowData => {
          const source = rowData?.source ? DataSource.fromObject(rowData.source) : null;
          const div = document.createElement('div');
  ...
  private showDataSourceConfigModal(dataSource: DataSource, onBack?: () => void, cancelTitle = 'Back') {
    const source = DataSource.fromObject(dataSource);

    this.sourceConfigModal.show(
      source,
      !!this.source,
      cancelTitle,
      async source => {
  ...
  private createNewDataSource() {
    ...
    this.sourceSelectionModal.show(source => this.showDataSourceConfigModal(source), allowClickBackDrop);
  }

  private changeDataSource(currentSource: DataSource) {
    const allowClickBackDrop = !!this.source;
    this.sourceSelectionModal.show(
      source =>
  ...
  private editDataSource(currentSource: DataSource) {
    if (currentSource.className === DataSourceType.Unknown) {
      return;
    }
  ...
  private get headerResolver(): HeaderResolver {
    return new HeaderResolverBuilder()
      .add(DataSourceType.Clickhouse, new ClickhouseHeaderHandler())
      .add(DataSourceType.Bigquery, new BigqueryHeaderHandler())
      .add(DataSourceType.MySQL, new MySQLHeaderHandler())
      .add(DataSourceType.Vertica, new VerticaHeaderHandler())
