...
import { ChartControlField, DatabaseSchema, Field, FieldDetailInfo } from '@core/common/domain';
...
export default class SelectFieldContext extends Vue {
  ...
  private currentDatabase: DatabaseSchema | null = _BuilderTableSchemaStore.databaseSchema;
  ...
  private async loadDatabaseSchema(dbName: string): Promise<DatabaseSchema> {
    if (dbName === _BuilderTableSchemaStore.databaseSchema?.name) {
      ...
      const databaseSchema = await DatabaseSchemaModule.handleGetDatabaseSchema(dbName);
      return databaseSchema;
    }
  }

  private renderSuggestFields(databaseSchema: DatabaseSchema, tblName: string) {
  ...
  private async initTablesOptions(dbName: string): Promise<void> {
    try {
      this.status = Status.Loading;
      const databaseSchema: DatabaseSchema = await this.loadDatabaseSchema(dbName);
  ...
  private async initChartControls() {
    try {
      ...
      this.chartControlList = _BuilderTableSchemaStore.tabControls.map(control => {
