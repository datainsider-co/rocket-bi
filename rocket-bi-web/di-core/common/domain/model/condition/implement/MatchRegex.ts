...
export class MatchRegex extends FieldRelatedCondition implements ValueCondition {
  className = ConditionType.MatchRegex;
  value: string;
  ...
  static fromObject(obj: MatchRegex): MatchRegex {
    const field = Field.fromObject(obj.field);
    const value = obj.value;
    return new MatchRegex(field, value, getScalarFunction(obj.scalarFunction));
  }
  ...
  getConditionTypes(): string[] {
    return [ConditionFamilyTypes.string, StringConditionTypes.matchesRegex];
  }
  ...
  toConditionData(groupId: number): ConditionData {
    ...
    return {
      ...
      subType: StringConditionTypes.matchesRegex,
      firstValue: this.value,
      secondValue: void 0,
      allValues: this.getValues(),
      currentInputType: InputType.text,
      filterModeSelected: FilterMode.selection,
      currentOptionSelected: StringConditionTypes.matchesRegex
    };
  }

  toString() {
    return `match regex ${this.value}`;
  }
}
