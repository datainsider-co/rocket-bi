...
import datainsider.jobscheduler.domain.job.{JobStatus, JobType, ReportType, TikTokAdsEndPoint, TikTokAdsJob, TikTokReport, TikTokTimeRange}
...
class TikTokAdsJobTest extends IntegrationTest with BeforeAndAfterAll {
  ...
  test("create multi job") {
    ...
    ReportType.values.foreach(report => {
      assert(jobs.exists(job => job.tikTokReport.get.reportType.equals(report.toString)))
    })
  }
