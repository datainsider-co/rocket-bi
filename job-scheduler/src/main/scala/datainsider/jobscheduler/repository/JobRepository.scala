...
trait JobRepository {
  ...
  def list(orgId: Long, from: Int, size: Int, sorts: Seq[SortRequest], keyword: Option[String]): Future[Seq[Job]]
    ...
    Future {
      ...
      val query =
        s"""
           ...
           |limit ? offset ?;
           |""".stripMargin
      client.executeQuery(query, orgId, s"%${keyword.getOrElse("")}%", size, from)(toJobs)
    }
    ...
    Future {
      val query =
        s"""
         ...
         |set name = ?, job_type = ?, last_modified = ?, sync_mode = ?, source_id = ?, last_successful_sync = ?, sync_interval_in_mn = ?, next_run_time = ?, last_sync_status = ?, current_sync_status = ?, destination_db = ?, destination_tbl = ?, job_data = ?
