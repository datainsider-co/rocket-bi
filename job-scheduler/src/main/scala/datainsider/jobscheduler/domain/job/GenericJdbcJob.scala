...
import datainsider.jobscheduler.domain.job.JobType.{GenericJdbc, JobType}
import datainsider.jobscheduler.domain.job.SyncMode.SyncMode
import datainsider.jobscheduler.domain.{GenericJdbcProgress, JobProgress}
    ...
    progress.jobStatus match {
      case JobStatus.Synced =>
        this.copy(
          ...
          lastSyncedValue = progress.asInstanceOf[GenericJdbcProgress].lastSyncedValue
        )
...
object GenericJdbcJob {
  def fromResultSet(rs: ResultSet): GenericJdbcJob = {
