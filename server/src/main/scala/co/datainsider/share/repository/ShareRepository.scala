...
case class MySqlShareRepository(client: JdbcClient, dbName: String, tblName: String, generator: IDGenerator[Int])
  ...
  private def countResourceSharingByUser(
      organizationId: Long,
      resourceType: String,
      username: String,
    ...
    client.executeQuery[Long](
      s"""
         ...
         |FROM ${getTable}
         |WHERE is_deleted = FALSE AND organization_id = ? AND resource_type = ? AND username = ? ${buildRootCondition(
        isRoot
      )}
         |""".stripMargin,
      organizationId,
  ...
  override def deleteByOrgId(organizationId: Long): Future[Boolean] = Future {
    val query = s"""
