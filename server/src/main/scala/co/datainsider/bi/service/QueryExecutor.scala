...
import co.datainsider.bi.util.Implicits.{FutureEnhance, ImplicitObject, NULL_VALUE, async}
import co.datainsider.bi.util._
import co.datainsider.common.client.exception.{BadRequestError, InternalError}
...
final class QueryExecutorImpl @Inject() (engineService: EngineService) extends QueryExecutor
    with Logging {

  override def parseQuery(orgId: Long, query: Query, useAliasName: Boolean): Future[String] = {
    for {
      engine <- engineService.get(orgId)
      sqlParser = new QueryParserImpl(engine.getSqlParser())
      sql <- sqlParser.parse(query, useAliasName)
    } yield sql
  }
  ...
  override def execute(orgId: Long, query: String, formatValues: Boolean): Future[DataTable] = {
    for {
      engine <- engineService.get(orgId)
      result: DataTable <- engine.execute(query, formatValues)
    } yield result
  }
  ...
  override def executeHistogramQuery(
    ...
    for {
      ...
      dataTable: DataTable <- engine.executeHistogramQuery(histogramSql)
    } yield dataTable
  }
