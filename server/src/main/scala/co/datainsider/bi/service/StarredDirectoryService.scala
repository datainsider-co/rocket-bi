...
trait StarredDirectoryService {
  ...
  def cleanup(organizationId: Long): Future[Boolean]
}
...
class StarredDirectoryServiceImpl @Inject() (
  ...
  override def list(
    ...
    for {
      ...
      directories <- directoryRepository.list(organizationId, staredDirectoryIds)
    } yield directories
  }
  ...
  override def cleanup(organizationId: DirectoryId): Future[Boolean] = {
    starredDirectoryRepository.deleteByOrgId(organizationId)
  }
}
