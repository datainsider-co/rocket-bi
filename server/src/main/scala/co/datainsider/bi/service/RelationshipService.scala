...
class RelationshipServiceImpl @Inject() (relationshipRepository: RelationshipRepository) extends RelationshipService {
...
//      allViews <- dashboardService.get(orgId, dashboardId).map(_.getAllQueryViews)
      ...
      currentRelationshipInfo match {
        case Some(relationshipInfo) =>
          relationshipInfo.copy(
            views = relationshipInfo.views,
            relationships = (relationshipInfo.relationships ++ globalRelationship.relationships).distinct
          )
        case None => RelationshipInfo(globalRelationship.views, globalRelationship.relationships)
      }
    }
  }
  ...
  override def cleanup(orgId: Long): Future[Boolean] = {
    deleteGlobal(orgId)
    // fixme: can't delete all relationship of organization
  }
}
