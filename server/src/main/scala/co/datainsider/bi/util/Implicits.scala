...
import co.datainsider.bi.domain.query.{Function, Select, TableField}
...
import co.datainsider.common.client.exception.{NotFoundError, UnAuthorizedError}
...
object Implicits {
  ...
  implicit class FutureOptionEnhanceLike[T](val fn: TwitterFuture[Option[T]]) extends AnyVal {
    def notNullOrEmpty: TwitterFuture[T] = {
      fn.flatMap({
        case Some(x) => TwitterFuture.value(x)
        case None    => TwitterFuture.exception(NotFoundError("Not found value"))
      })
    }

    def notNullOrEmpty(msg: String): TwitterFuture[T] = {
      fn.flatMap({
        case Some(x) => TwitterFuture.value(x)
        case None    => TwitterFuture.exception(NotFoundError(msg))
      })
    }
  }
