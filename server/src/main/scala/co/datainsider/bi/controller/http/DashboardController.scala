...
//  filter[MustLoggedInFilter]
//    .get("/dashboards/template/:id") { request: Request =>
//      Profiler(s"/dashboards/template/:id") {
//        val id: Long = request.getLongParam("id")
//        templateService.get(id)
//      }
//    }
//
//  filter[MustLoggedInFilter]
//    .get("/dashboards/template/list") { request: ListTemplateDashboardsRequest =>
//      Profiler(s"/dashboards/template/list") {
//        templateService.list(request.keyword, request.from, request.size)
//      }
//    }
//
//  filter[MustLoggedInFilter]
//    .filter[AccessTokenFilter]
//    .post("/dashboards/template") { request: CreateTemplateRequest =>
//      Profiler(s"/dashboards/template") {
...
//  filter[MustLoggedInFilter]
//    .filter[AccessTokenFilter]
//    .delete("/dashboards/template/:id") { request: Request =>
//      Profiler(s"/dashboards/template/:id DELETE") {
//        val id: Long = request.getLongParam("id")
//        templateService.delete(request.getOrganizationId(), id)
//      }
//    }
//
//  filter[MustLoggedInFilter]
//    .filter[AccessTokenFilter]
//    .put("/dashboards/template/:id") { request: UpdateTemplateRequest =>
//      Profiler(s"/dashboards/template/:id PUT") {
...
//      directoryFilter.requireTokenPermission("create", "parent_directory_id")
//    )
//  ).post("/dashboards/create_from_template") { request: CreateFromTemplateRequest =>
//    Profiler(s"/dashboards/create_from_template")
//    UserActivityTracker(
//      request = request.request,
//      actionName = request.getClass.getSimpleName,
//      actionType = ActionType.Create,
//      resourceType = ResourceType.Dashboard,
//      resourceId = null,
//      description = s"create new dashboard '${request.name}' from template '${request.templateId}'"
//    ) {
//      request.parentDirectoryId match {
//        case Shared => Future.exception(UnsupportedError("unsupported create dashboard at root shared"))
//        case MyData =>
//          for {
//            rootId <- getRootId(request.request)
//            dashboard <- templateService.createFromTemplate(request.copy(parentDirectoryId = rootId))
//          } yield dashboard
//        case _ => templateService.createFromTemplate(request)
//      }
//    }
//  }
//
...
//      dashboardFilter.requireTokenPermission(action = "edit", dashboardParamName = "id")
//    )
//  ).post(s"/dashboards/:id/connect_datasource") { request: Request =>
//    Profiler(s"/dashboards/:id/connect_datasource") {
//      val dashboardId = request.getLongParam("id")
//      UserActivityTracker(
//        request = request.request,
//        actionName = request.getClass.getSimpleName,
//        actionType = ActionType.Update,
...
//        description = s"apply connect datasource of dashboard ${dashboardId}"
//      ) {
//        templateService.applyConnectDataSource(request.getOrganizationId(), dashboardId).map(toResponse)
//      }
//    }
//  }
