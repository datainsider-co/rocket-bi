...
object VerticaParser extends SqlParser {
  ...
  def toQueryString(scalarFn: ScalarFunction, field: String): String = {
    scalarFn match {
      ...
      case ToDateTime(_, _) => s"$field::DATETIME"

      case SecondsToDateTime(_, _) => s"TO_TIMESTAMP($field)"
      case MillisToDateTime(_, _)  => s"TO_TIMESTAMP($field / 1000)"
      case NanosToDateTime(_, _)   => s"TO_TIMESTAMP($field / 1000000)"
      ...
      case PastNYear(unit, _, _)    => s"$field - INTERVAL '$unit YEAR'"
      case PastNQuarter(unit, _, _) => s"$field - INTERVAL '$unit QUARTER'"
      case PastNMonth(unit, _, _)   => s"$field - INTERVAL '$unit MONTH'"
