...
trait DirectoryRepository {
  ...
  def deleteByOrgId(organizationId: OrganizationId): Future[Boolean]
}
...
class MySqlDirectoryRepository @Inject() (
  ...
  override def deleteByOrgId(organizationId: OrganizationId): Future[Boolean] =
    Future {
      val query = s"""
           |delete from $dbName.$tblName
           |where org_id = ?;
           |""".stripMargin
      client.executeUpdate(query, organizationId) >= 1
    }
}
