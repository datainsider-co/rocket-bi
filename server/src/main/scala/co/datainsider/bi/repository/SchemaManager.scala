...
/** *
  ...
  */
@deprecated("use MySqlSchemaManager instead") // TODO: migrate this class to MySqlSchemaManager
class BIServiceSchemaManager @Inject() (
    ...
    Future
      .collect(
        Seq(
          ...
          ensureConnectionsTable(tblConnectionName)
        )
      )
  ...
  def ensureConnectionsTable(tableName: String): Future[Boolean] =
    Future {
      val requiredFields = Set("org_id", "data", "created_at", "updated_at", "created_by", "updated_by")
      if (isTblExist(dbName, tableName)) {
        isAllTblFieldValid(s"$dbName.$tableName", requiredFields)
      } else createDataSourceTable(tableName)
    }

  private[BIServiceSchemaManager] def isTblExist(dbName: String, tblName: String): Boolean = {
  ...
  def createDataSourceTable(tblName: String): Boolean = {
    client.executeUpdate(s"""
