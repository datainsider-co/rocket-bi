...
trait SchemaService {
  ...
  def createDatabaseFromSchema(dbSchema: DatabaseSchema): Future[DatabaseSchema]
  ...
  override def listDatabases(organizationId: Long, request: ListDBRequest): Future[ListDatabaseResponse] =
    Profiler(s"$clazz::listDatabases") {
      for {
        ...
        schemaInfoList <- toShortSchemaInfo(organizationId, permittedDatabases)
      } yield ListDatabaseResponse(schemaInfoList, schemaInfoList.size)
    }
