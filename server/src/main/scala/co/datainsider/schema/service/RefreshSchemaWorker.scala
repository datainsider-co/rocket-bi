...
trait RefreshSchemaWorker {
  ...
  /**
    ...
    */
  def run(orgId: Long, reportFn: (RefreshSchemaStage) => Unit): Unit
...
class RefreshSchemaWorkerImpl(
  ...
  override def run(orgId: Long, report: (RefreshSchemaStage) => Unit): Unit = {
    if (!isRunning(orgId)) {
      try {
        setRunning(orgId, isRunning = true)
        process(orgId, report)
      } finally {
  ...
  private def process(orgId: Long, report: RefreshSchemaStage => Unit): Unit = {
    try {
      logger.info(s"${Thread.currentThread().getName}Start refresh schema for source: $orgId")
      ensureRunning(orgId)
      val engine: Engine = engineService.get(orgId).syncGet()
      testConnection(orgId, engine, report)
      ensureRunning(orgId)
      scanAndUpdateDatabases(orgId, engine.getDDLExecutor(), report)
      report(
        RefreshSchemaStage(
          orgId = orgId,
