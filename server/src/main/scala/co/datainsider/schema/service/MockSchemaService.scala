...
import co.datainsider.schema.domain.responses.{FullSchemaInfo, ListDatabaseResponse, TableExpressionsResponse}
...
case class MockSchemaService() extends SchemaService {
  ...
  override def createDatabaseFromSchema(dbSchema: DatabaseSchema): Future[DatabaseSchema] = {
    Future {
  ...
  override def getDatabases(organizationId: Long): Future[Seq[DatabaseShortInfo]] =
    Future {
      databases.values.toSeq.asDatabaseShortInfos()
    }
  ...
  override def detectColumns(organizationId: Long, query: String): Future[Array[Column]] = Future.value(Array.empty)
