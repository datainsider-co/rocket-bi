...
import co.datainsider.common.client.exception.{BadRequestError, UnsupportedError}
...
object ClickHouseDDLConverter extends Logging {
  ...
  def toCreateShardTableDDL(tableSchema: TableSchema, clusterName: Option[String]): String = {
    ...
    s"""
       |CREATE TABLE IF NOT EXISTS `${tableSchema.dbName}`.`${tableSchema.defaultShardTblName}` ${ON_CLUSTER(
      clusterName
    )} (
       | ${toMultiColumnDDL(tableSchema.columns)}
  ...
  def toInsertSQL(
    ...
    val privateKey = ZConfig.getString("database.clickhouse.encryption.key")
