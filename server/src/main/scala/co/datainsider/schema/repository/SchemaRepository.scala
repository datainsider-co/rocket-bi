...
case class SchemaRepositoryImpl @Inject() (
  ...
  private def resolveDDLExecutor(organizationId: Long): Future[DDLExecutor] = {
    engineService.get(organizationId).map(_.getDDLExecutor())
  }
  ...
  override def detectExpressionType(
    ...
    for {
      engine <- engineService.get(organizationId)
      column: Column <- engine.detectExpressionColumn(dbName, tblName, newExpr, existingExpressions)
  ...
  override def detectAggregateExpressionType(
    ...
    for {
      engine <- engineService.get(organizationId)
      column <- engine.detectAggregateExpressionColumn(dbName, tblName, newExpr, existingExpressions)
    } yield Column.getCustomClassName(column)
  }
