...
case class MockSchemaClientService() extends SchemaClientService {
  ...
  override def isDbExists(organizationId: Long, dbName: String, useDdlQuery: Boolean): Future[Boolean] = Future.True
  ...
  override def deleteDatabase(organizationId: Long, dbName: String): Future[Boolean] = Future.True

  override def createTableSchema(schema: TableSchema): Future[TableSchema] = Future.value(schema)

  override def createDatabaseFromSchema(db: DatabaseSchema): Future[DatabaseSchema] = Future.value(db)

  override def createTableFromQueryInfo(orgId: Long, tableInfo: TableFromQueryInfo): Future[TableSchema] =
    Future.value(tableInfo.toTableSchema(orgId, Array.empty))
}
