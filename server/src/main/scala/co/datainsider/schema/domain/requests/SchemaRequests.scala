...
import co.datainsider.schema.domain.{DatabaseSchema, TableSchema, TableType, TableTypeRef}
...
case class CreateTableFromQueryRequest(
  ...
  def toTableFromQueryInfo: TableFromQueryInfo =
    TableFromQueryInfo(
      ...
      aliasDisplayNames = Seq.empty
    )
}

case class DetectColumnsRequest(@NotEmpty query: String, @Inject request: Request = null) extends LoggedInRequest
...
case class TableFromQueryInfo(
  ...
  private def applyAliasDisplayNames(columns: Array[Column], displayNames: Seq[String]): Array[Column] = {
  ...
  def toTableSchema(organizationId: Long, columns: Array[Column]): TableSchema = {
    val newColumns: Array[Column] = applyAliasDisplayNames(columns, aliasDisplayNames)
    TableSchema(
      name = tblName,
      dbName = dbName,
