...
import co.datainsider.common.client.exception.{InternalError, QuotaExceedError, VerificationCodeInvalidError}
...
case class EmailVerifyService @Inject() (
  ...
  override def sendForgotPasswordCode(organizationId: Long, email: String): Future[Boolean] = {
    for {
      ...
      case true => r
      case _    => throw InternalError(s"Can't send email to $email.")
    }
  }
  ...
  override def createVerifyCode(email: String): Future[String] = {
  ...
  override def deleteVerifyCode(receiver: String): Future[Unit] = Future {
    val key = buildVerifyCodeKey(receiver)
