...
import co.datainsider.caas.user_profile.domain.org.OrgMember
...
object MySqlOrganizationMemberRepository {
  private val INSERT_SQL =
    "INSERT INTO caas.organization_members (organization_id, username, added_by, added_time) VALUES (?, ?, ?, ?)"
  private val SELECT_BY_ORG_ID_USERNAME_SQL =
    "SELECT organization_id, username, added_by, added_time FROM caas.organization_members WHERE organization_id=? AND username=?"
  private val SELECT_BY_USERNAME_SQL =
    "SELECT organization_id, username, added_by, added_time FROM caas.organization_members WHERE username=?"
  private val COUNT_BY_ORG_ID_USERNAME_SQL =
    "SELECT COUNT(organization_id) FROM caas.organization_members WHERE organization_id=? AND username=?"
  private val DELETE_BY_ORG_ID_USERNAME_SQL =
    "DELETE FROM caas.organization_members WHERE organization_id=? AND username=?"
}
