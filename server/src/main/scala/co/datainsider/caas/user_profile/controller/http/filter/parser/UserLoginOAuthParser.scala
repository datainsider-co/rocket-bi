...
import co.datainsider.common.client.exception.{EmailNotExistedError, RegistrationRequiredError, UnsupportedError}
...
case class LoginOAuthRequest(oauthType: String, id: String, token: String, oauthInfo: OAuthInfo) {
  ...
  def buildProfile(): UserProfile = {
    UserProfile(
      ...
      fullName = Option(oauthInfo.name),
      lastName = oauthInfo.familyName,
      firstName = oauthInfo.givenName,
      email = Option(oauthInfo.email),
      avatar = Option(oauthInfo.avatarUrl)
    )
  }
}
