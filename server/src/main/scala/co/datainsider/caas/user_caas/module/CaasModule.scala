...
import co.datainsider.caas.user_caas.service._
...
object CaasModule extends TwitterModule {
  ...
  protected override def configure(): Unit = {
    ...
    bind[CaasService].to[CaasServiceImpl].asEagerSingleton()
    bind[SessionService].to[SessionServiceImpl].asEagerSingleton()

    bind[UserService].to[UserServiceImpl].asEagerSingleton()
    bind[RoleService].to[RoleServiceImpl].asEagerSingleton()
    bind[OrgAuthorizationService].to[OrgAuthorizationServiceImpl].asEagerSingleton()
  }
