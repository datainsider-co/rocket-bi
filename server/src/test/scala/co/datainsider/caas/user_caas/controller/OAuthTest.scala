...
import co.datainsider.caas.login_provider.domain.{FbOAuthConfig, GoogleOAuthConfig}
import co.datainsider.caas.login_provider.repository.{FacebookOAuthProvider, GoogleOAuthProvider}
...
object GoogleOAuthTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val obj = GoogleOAuthProvider(GoogleOAuthConfig(true, whitelistEmail = Seq.empty, clientIds = Set(token)))
      .getOAuthInfo(id, token)
...
object GoogleOAuthFailedTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val obj = GoogleOAuthProvider(GoogleOAuthConfig(true, whitelistEmail = Seq.empty, clientIds = googleAppId))
      .getOAuthInfo(id, token)
    println(JsonParser.toJson(obj))
  }
}
