...
class PostgreSqlEngineTest extends BasePostgreSqlTest {
  ...
  override def beforeAll(): Unit = {
    ...
    await(engine.getDDLExecutor().createTable(studentsSchema))
  }
  ...
  test("test detect expression column with invalid expression") {
    ...
    assertFailedFuture[DbExecuteError](
      engine.detectExpressionColumn(dbName, tblName, expression, Map.empty)
    )
  }
  ...
  test("test write data") {
    ...
    await(engine.write(studentsSchema, records))
    val query =
      s"""SELECT *
