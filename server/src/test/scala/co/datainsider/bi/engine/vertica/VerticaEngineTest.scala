...
import co.datainsider.bi.domain.{SshConfig, SshKeyPair}
...
class VerticaEngineTest extends BaseVerticaTest {
  ...
  test("test export csv") {
    ...
    val destPath = await(engine.exportToFile(sql, s"${baseDir}/sales.csv", FileType.Csv))
  ...
  test("test export excel") {
    ...
    val destPath = await(engine.exportToFile(sql, s"${baseDir}/sales.xlsx", FileType.Excel))
    val file = new File(destPath)
    assert(file.exists())
    assert(file.length() > 0)
    file.delete()
  ...
  test("test detect expression column success") {
    ...
    val column = await(engine.detectExpressionColumn(dbName, tblName, expression, Map.empty))
    assert(column != null)
  ...
  test("test detect expression column with invalid expression") {
    val expression = "Total_Profit + 100 +"
    assertFailedFuture[DbExecuteError](engine.detectExpressionColumn(dbName, tblName, expression, Map.empty))
  }
  ...
  test("test query histogram") {
    ...
    val dataTable: DataTable = await(engine.execute(histogramSql))
  ...
  test("test write data to vertica") {
    ...
    await(engine.write(testWriteTableSchema, records))
    val query = s"""SELECT *
  ...
  test("connect using ssh") {
    ...
    val originConnection = source.copyHostPorts(verticaServiceName, Map(source.port -> originPort))
    println(s"open tunnel from localhost -> ${tunnelConfig.host}:${tunnelConfig.port}")
    val session = TunnelSession
      .newBuilder()
      .setKeyPair(keypair)
      .setTunnelConnection(originConnection.copy(tunnelConfig = Some(tunnelConfig)))
      .build()
    Using(session)(session => {
      session.open()
      val tunnelConnection = source.copyHostPorts(session.getMappedHost(), session.getMappedPortAsMap())
