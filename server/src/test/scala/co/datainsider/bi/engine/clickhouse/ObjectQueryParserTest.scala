...
import co.datainsider.bi.domain.{ClickhouseConnection, RlsCondition}
import co.datainsider.bi.domain.chart.{NumberChartSetting, TableColumn}
import co.datainsider.bi.domain.query.{TableField, _}
...
class ObjectQueryParserTest extends IntegrationTest with BeforeAndAfterAll {
  ...
  test("test select function") {
    ...
    val results = DbTestUtils.execute(client, query)
    assert(results != null)
 ...
 val query = Await.result(parser.parse(objQuery, Array.empty))
  ...
  test("test add filters to object query") {
    ...
    val queryResp: SqlQueryResponse = DbTestUtils.execute(client, query)
  ...
  test("object query preview chart with isLimit = true") {
    ...
    val results = DbTestUtils.execute(client, limitedSql)
    assert(results != null)
    println(results)
  }
  ...
  test("test parse object query with rls") {
    ...
    val results = DbTestUtils.execute(client, sql)
  ...
  private def getCustomersFromCsv: mutable.Map[(String, String), String] = {
    val filePath: String = getClass.getClassLoader.getResource("datasets/test-data/customers.csv").getFile

    val customersCsv = Source.fromFile(filePath)
  ...
  private def getNumOrders: Int = {
    val filePath: String = getClass.getClassLoader.getResource("datasets/test-data/orders.csv").getFile

    val ordersCsv = Source.fromFile(filePath)
