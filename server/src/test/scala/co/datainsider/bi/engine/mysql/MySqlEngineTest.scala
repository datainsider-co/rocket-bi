...
class MySqlEngineTest extends BaseMySqlTest {
  ...
  test("test export csv") {
    ...
    val destPath = engine.exportToFile(sql, csvPath, FileType.Csv).syncGet()
  ...
  test("test detect aggregate expression column success") {
    ...
    val column = await(engine.detectAggregateExpressionColumn(dbName, tblName, expression, Map.empty))
    assert(column != null)
    assert(column.getClass == classOf[DoubleColumn])
  }
  ...
  test("test detect aggregate expression column with invalid expression") {
    ...
    assertFailedFuture[DbExecuteError](
      engine.detectAggregateExpressionColumn(dbName, tblName, expression, Map.empty)
    )
  }
  ...
  test("test query histogram") {
    ...
    val histogramSql = MySqlParser.toHistogramSql(fieldName, baseSql, 5)
  ...
  test("test write data") {
    ...
    engine.write(schema, records).syncGet()
  ...
  def assert(actualList: Seq[Any], expectedList: Seq[Any]): Unit = {
    assert(JsonParser.toJson(actualList, false) == JsonParser.toJson(expectedList, false))
  }
}
