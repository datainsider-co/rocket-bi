...
class BigQueryParserTest extends Test {
  ...
  val engine: BigQueryEngine = bigqueryEngineCreator.create(bqSource)
  val ddlExecutor: DDLExecutor = engine.getDDLExecutor()
  val bigqueryClient: BigQueryClient = engine.client
