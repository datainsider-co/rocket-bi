...
import co.datainsider.bi.engine.posgresql.{PostgreSqlConnection, PostgreSqlEngine, PostgreSqlEngineFactory}
...
/**
  ...
  */
class BasePostgreSqlTest extends IntegrationTest {
  override protected val injector: Injector = TestInjector(TestContainerModule).create

  val postgresSource: PostgreSqlConnection = TestContainerModule.providesPostgreSqlSource()
  val engineCreator = new PostgreSqlEngineFactory()

  val engine: PostgreSqlEngine = engineCreator.create(postgresSource)
