...
class DeletedDirectoryServiceTest extends IntegrationTest {
  ...
  override protected def injector: Injector =
    TestInjector(TestModule, MockCaasClientModule, MockShareModule, TestBIClientModule, TestContainerModule, TestCommonModule)
      .newInstance()
  ...
  private val mockLoggedInReq: Request = MockUserContext.getLoggedInRequest(orgId, username)
  ...
  test("insert mock data") {
    ...
    val isSuccess = await(repository.multiInsert(orgId, directories))
    assert(isSuccess)
  }

  test("list root directories") {
    val rootDirectories =
      await(deletedDirectoryService.listRootDirectories(ListDirectoriesRequest(request = mockLoggedInReq)))
    assert(rootDirectories.size >= 2)
    assert(rootDirectories.forall(_.parentId == -1))
    assert(rootDirectories.forall(_.ownerId == username))
    val rootDirectoriesIds = rootDirectories.map(_.id).toSet
    assert(rootDirectoriesIds.contains(rootDir1.id))
    assert(rootDirectoriesIds.contains(rootDir2.id))
  }

  test("list directories by parent id") {
    val childDirectories =
      await(
        deletedDirectoryService.listDirectories(
          ListDirectoriesRequest(parentId = Some(rootDir1.id), request = mockLoggedInReq)
        )
      )
  ...
  test("restore directory") {
    ...
    val childDirectories =
      await(
        deletedDirectoryService.listDirectories(
          ListDirectoriesRequest(parentId = Some(rootDir2.id), request = mockLoggedInReq)
        )
      )
    assert(childDirectories.size == 3)
