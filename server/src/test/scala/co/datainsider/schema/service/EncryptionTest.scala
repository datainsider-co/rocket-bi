...
class EncryptionTest extends IntegrationTest {
  override protected val injector: Injector =
    TestInjector(MockCaasClientModule, SchemaTestModule, TestContainerModule, TestModule, TestBIClientModule, TestCommonModule)
      .newInstance()
  ...
  protected val client: JdbcClient = clickhouseEngine.client
  ...
  override def beforeAll(): Unit = {
    super.beforeAll()
    Try(await(schemaManager.dropDatabase(1, testDbName)))
  ...
  override def afterAll(): Unit = {
    super.afterAll()
    await(schemaManager.dropDatabase(1, testDbName))
  }
