CREATE DATABASE IF NOT EXISTS `etl_test`;

CREATE TABLE IF NOT EXISTS `etl_test`.`job` (
...
CREATE TABLE IF NOT EXISTS `etl_test`.`deleted_job` (
    `organization_id` BIGINT,
    `id` BIGINT AUTO_INCREMENT PRIMARY KEY,
    `display_name` TEXT,
    `operators` LONGTEXT,
...
CREATE TABLE IF NOT EXISTS `etl_test`.`job_history` (
...
CREATE TABLE IF NOT EXISTS `etl_test`.`share_info`(
