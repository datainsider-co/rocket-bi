...
import datainsider.user_caas.domain.{Page, PasswordMode}
...
trait UserService {
  ...
  def updatePassword(organizationId: Long, username: String, oldPassword: String, newPassword: String): Future[Boolean]
...
case class UserServiceImpl @Inject() (
  ...
  override def resetPassword(
    ...
    Future {
      val passwordHashed = getPasswordHash(passwordMode.getOrElse(PasswordMode.Raw), password)
      userRepository.resetPassword(organizationId, username, passwordHashed)
    }
  ...
  override def updatePassword(
    ...
    Future {
      val oldPasswordHashed = getPasswordHash(PasswordMode.Raw, oldPassword)
      val newPasswordHashed = getPasswordHash(PasswordMode.Raw, newPassword)

      userRepository.resetPassword(organizationId, username, oldPasswordHashed, newPasswordHashed)
    }
  ...
  override def getListUserByUsernames(organizationId: Long, usernames: Seq[String]): Future[Seq[User]] = Future{
