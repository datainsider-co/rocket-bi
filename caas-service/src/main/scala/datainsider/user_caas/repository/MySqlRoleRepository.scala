...
import datainsider.client.domain.user.RoleInfo
...
object MySqlRoleRepository {
  final private val SQL_INSERT_ROLE = "INSERT INTO role (organization_id, role_id, role_name) VALUES (?, ?, ?)"
  final private val SQL_UPDATE_ROLE = "UPDATE role SET role_name=? WHERE organization_id=? AND role_id=?"
  final private val SQL_ROLE_FIELD = ",(?, ?) "
  final private val SQL_DELETE_ROLE = "DELETE FROM role WHERE organization_id=? AND role_id=?"
  final private val SQL_SELECT_ROLE =
    "SELECT organization_id, role_id, role_name FROM role WHERE organization_id=? AND role_id=?"
  final private val SQL_SELECT_ALL_ROLE = "SELECT organization_id, role_id, role_name FROM role WHERE organization_id=?"
  final private val SQL_INSERT_ROLE_PERM =
    "INSERT INTO role_permissions (organization_id, role_id, permission) VALUES (?, ?, ?)"
  ...
  final private val SQL_DELETE_ROLE_PERM_LIST =
    "DELETE FROM role_permissions WHERE (organization_id, role_id, permission) IN ((?, ?, ?))"
  final private val SQL_SELECT_ROLE_PERM_SET =
    "SELECT role_id, permission FROM role_permissions WHERE organization_id=? AND role_id=?"
  final private val SELECT_ROLE_PERMS_BY_NAME_SQL =
    """
      |SELECT permission
      |FROM role_permissions,role
      |WHERE role.organization_id=? AND role_permissions.role_id = role.role_id AND role.role_name=?
      |""".stripMargin
  ...
  final private val SELECT_ROLE_PERMS_BY_ROLE_IDS_SQL =
    """
      |SELECT permission
      |FROM role_permissions,role
      |WHERE role.organization_id=? AND role_permissions.role_id = role.role_id AND role.role_id IN (%s)
      |""".stripMargin
}

case class MySqlRoleRepository(client: JdbcClient) extends RoleRepository with Logging {
  ...
  private def insertRoles(organizationId: Long, roleIds: Seq[Int], roleNames: Seq[String]): Int = {
    ...
    val records = roleIds.zip(roleNames).map {
      case (id, name) => Seq(organizationId, id, name)
    }
  ...
  override def addRolePermission(organizationId: Long, roleId: Int, permissions: Set[String]): Unit = {
    ...
    val records = permissions.map(perm => Seq(organizationId, roleId, perm)).toSeq
  ...
  override def deleteRolePermission(organizationId: Long, roleId: Int, permissions: Set[String]): Unit = {
    if (permissions == null || permissions.isEmpty) return
    val records = permissions.map(permission => Seq(organizationId, roleId, permission)).toSeq
