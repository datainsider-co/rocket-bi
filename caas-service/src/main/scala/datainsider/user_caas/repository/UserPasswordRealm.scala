...
import datainsider.user_caas.domain._
...
object UserPasswordRealm {
  ...
  val AUTHENTICATION_QUERY = "SELECT password FROM user WHERE username=? AND is_active=1"
  val USER_ROLE_QUERY =
    "SELECT role.role_name FROM role,user_roles,user WHERE role.role_id=user_roles.role_id AND user_roles.username=user.username AND user.username=?"
  val PERMISSION_QUERY =
    "SELECT permission FROM role_permissions,role WHERE role_permissions.role_id=role.role_id AND role.role_name=?"
}
