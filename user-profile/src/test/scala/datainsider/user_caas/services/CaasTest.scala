...
import datainsider.user_caas.service.{Caas, CaasService, OrgAuthorizationService, UserService}
...
class CaasTest extends DataInsiderIntegrationTest {
  ...
  test("Assign permission access database ID: 1") {
    ...
    // wrong case
    val permissions = Seq("*:database:access:1", s"${organizationId}:*:access:1", s"${organizationId}:database:*:1", s"${organizationId}:database:access:*")
  ...
  test("Assign permission access * for ID: 2") {
    ...
    // happy case
    val isPermitted = caas.isPermitted(organizationId, username, permission, "0:directory:access:2", "0:database:access:2").forall(_ == true)
    assertResult(isPermitted)(true)

    // wrong case
    val permissions = Seq("*:database:access:2", s"0:database:*:2", "0:*:access:3", "0:database:access:3", s"0:database:access:*")
    val results = caas.isPermitted(organizationId, username, permissions: _*)
    val isAllNotPermitted = results.forall(_ == false)
    assertResult(isAllNotPermitted)(true)
  }
  ...
  test("Assign permission access database for any id") {
    ...
    // happy case
    val isPermitted = caas.isPermitted(organizationId, username, permission, "0:database:access:2", "0:database:access:3").forall(_ == true)
    assertResult(isPermitted)(true)
  ...
  test("Assign permission *:*:*") {
    ...
    // happy case
    val isPermitted = caas.isPermitted(organizationId, username, permission, "database:access:2", "*:access:3", "*:view:*", "1:database:access:1", "run:database:access:1").forall(_ == true)
    assertResult(isPermitted)(true)
  ...
  test("Wildcard 0:*:* match with 0:setting:edit:schema") {
    ...
    assertResult(isInserted)(true)
    // true case
    val isPermitted = caas.isPermitted(organizationId, username, permission, "0:setting:edit:schema", "0:setting:edit", "0:setting:edit:schema:*", "0:*").forall(_ == true)
    assertResult(isPermitted)(true)
    // false case
