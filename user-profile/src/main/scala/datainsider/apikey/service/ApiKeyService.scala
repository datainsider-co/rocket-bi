package datainsider.apikey.service
...
import datainsider.apikey.repository.ApiKeyRepository
import datainsider.client.domain.ThriftImplicit.RichUser
import datainsider.client.domain.user.{LoginResponse, User, UserProfile}
import datainsider.client.util.ZConfig
import datainsider.user_caas.domain.UserType
import datainsider.user_caas.service.{OrgAuthorizationService, UserService}
  ...
  override def updateApiKey(request: UpdateApiKeyRequest): Future[Boolean] = {
    for {
      ...
      isUpdatedPermissionSuccess <- orgAuthorizationService.changePermissions(
        ...
        request.apiKey,
        request.getIncludesPermissions,
        request.getExcludePermissions
      )
      isUpdateInfoSuccess <- apiKeyRepository.update(
        request.getOrganizationId(),
        request.apiKey,
        request.displayName,
        request.expiredTimeMs.getOrElse(currentApiKeyInfo.expiredTimeMs),
