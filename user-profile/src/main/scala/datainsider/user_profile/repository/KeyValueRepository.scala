...
object SSDBKeyValueRepository {
  ...
  implicit class KeyValueRepositoryAsync[K, V](repository: KeyValueRepository[K, V]) {
    ...
    def asyncPut(k: K, v: V, ttl: Option[Int] = None): Future[Unit] = async {
      repository.put(k, v, ttl)
    }

    def asyncExpire(k: K, ttl: Int): Future[Unit] = async {
      repository.expire(k, ttl)
    }

    def asyncExists(k: K): Future[Boolean] = async {
      repository.exists(k)
    }

    def asyncDelete(k: K): Future[Unit] = async {
      repository.delete(k)
    }

    def asyncIncr(k: K, num: Int): Future[V] = async {
      repository.incr(k, num)
    }


    def asyncGet(k: K): Future[Option[V]] = async {
      repository.get(k)
    }

    def asyncGets(k: Seq[K]): Future[Map[K, V]] = async {
      repository.gets(k)
    }

    def asyncTimeLeft(k: K): Future[Option[Int]] = async {
...
abstract class SSDBKeyValueRepository[K, V](ssdb: SSDB) extends KeyValueRepository[K, V] {
  ...
  override def put(k: K, v: V, ttl: Option[Int] = None): Unit = {
    ...
    resp.ok() match {
      case true => ttl match {
        case Some(ttl) => ssdb.expire(k, ttl) match {
          case r if r.asInt() > 0 =>
          case _ =>
            ssdb.del(k)
