...
) extends PermissionAccessFilters.DeleteAccessFilter {
  ...
  private def isPermissionPermitted(request: Request)= {
    ...
    caasService
      ...
      .isPermitted(
        ...
        PermissionProviders.perm.withOrganizationId(organizationId).delete()
      ).map(PermissionResult(_, "No permission to delete permissions."))
  }
