...
import { CheckProgressResponse, EtlOperator, GetDataOperator, Position, PositionValue } from '@core/DataCook';
import { DatabaseSchema, TableSchema } from '@core/domain';

export type TMapRenderTableItem = { [key: string]: HTMLElement };
export type CustomLeaderLine = LeaderLine & { $el: HTMLElement; deleted: boolean };

export type TModel = {
  dbName: string;
  getDataItems: GetDataOperator[];
  notGetDataItems: EtlOperator[];
  map: Record<string, EtlOperator>;
  mapTable: Record<string, EtlOperator>;
  mapLoading: Record<string, boolean>;
  mapPreviewId: Record<string, number>;
