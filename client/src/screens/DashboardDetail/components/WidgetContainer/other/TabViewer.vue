...
import { DashboardModule, WidgetModule } from '@/screens/DashboardDetail/stores';
...
import { DIMap, Position, TabWidget, Widget, WidgetId, Tab } from '@core/domain';
...
export default class TabViewer extends Vue {
  ...
  private async handleDeleteChart(tabIndex: number) {
    ...
    this.$root.$emit(DashboardEvents.RemoveChartFromTab, this.widget, tabIndex, (widgetIds: WidgetId[]) => {
      widgetIds.forEach(id => this.gridstacks[tabIndex].removeItemById(id));
    });
  }
