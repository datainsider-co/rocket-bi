...
export class Equal extends FieldRelatedCondition implements ValueCondition {
  className = ConditionType.Equal;
  value: string;
  ...
  static fromObject(obj: Equal): Equal {
    const field = Field.fromObject(obj.field);
    const value = obj.value;
    return new Equal(field, value, getScalarFunction(obj.scalarFunction));
  }
